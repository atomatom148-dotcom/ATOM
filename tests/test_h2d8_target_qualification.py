...
def test_skips_honest_market_data_rejection_in_fixed_order(monkeypatch):
    payloads = {day: _payload(day) for day, _run_id in qualifier.FROZEN_TARGETS}
    payloads["2026-06-16"] = _payload("2026-06-16", rejected=True)
    calls = []
    receipt = _execute(monkeypatch, payloads, calls=calls)

    assert calls == ["2026-06-16", "2026-06-25", "2026-06-29"]
    assert receipt["selected_sessions"] == ["2026-06-25", "2026-06-29"]
...
def test_independent_five_second_gate_rejects_six_seconds(monkeypatch):
    payloads = {day: _payload(day) for day, _run_id in qualifier.FROZEN_TARGETS}
    payloads["2026-06-16"] = _payload("2026-06-16", gap_seconds=6)
...
def test_over_gap_receipt_still_requires_valid_lineage(monkeypatch):
    payloads = {day: _payload(day) for day, _run_id in qualifier.FROZEN_TARGETS}
    payloads["2026-06-16"] = _payload("2026-06-16", gap_seconds=6)
    payloads["2026-06-16"]["configuration_digest"] = "0" * 64
    with pytest.raises(
...
def test_qualifying_receipt_requires_expected_preflight_run_id(monkeypatch):
    payloads = {day: _payload(day) for day, _run_id in qualifier.FROZEN_TARGETS}
    payloads["2026-06-16"]["replay_run_id"] = "h1-preflight-wrong"
    with pytest.raises(
        qualifier.TargetQualificationFailure, match="LINEAGE_OR_RECEIPT_ERROR",
    ):
        _execute(monkeypatch, payloads)
...
def test_rejected_receipt_with_lineage_drift_fails_closed(monkeypatch):
    payloads = {day: _payload(day) for day, _run_id in qualifier.FROZEN_TARGETS}
    payloads["2026-06-16"] = _payload("2026-06-16", rejected=True)
    payloads["2026-06-16"]["runner_version"] = "drifted"
    with pytest.raises(
...
def test_default_preflight_enforces_and_clears_deadline(monkeypatch):
    ...
    with pytest.raises(
        ...
        qualifier._run_preflight(object(), "2026-06-16", 2.5)
    assert timer_calls == [
...
def test_scope_stays_read_only_and_outside_v9():
    ...
    assert qualifier.FROZEN_TARGETS == (
        ("2026-06-16", "h2d-2026-06-16"),
        ("2026-06-25", "h2d-2026-06-25"),
        ("2026-06-29", "h2d-2026-06-29"),
        ("2026-06-30", "h2d-2026-06-30"),
        ("2026-07-01", "h2d-2026-07-01"),
        ("2026-07-02", "h2d-2026-07-02"),
        ("2026-07-06", "h2d-2026-07-06"),
        ("2026-07-07", "h2d-2026-07-07"),
        ("2026-07-08", "h2d-2026-07-08"),
        ("2026-07-09", "h2d-2026-07-09"),
        ("2026-07-10", "h2d-2026-07-10"),
        ("2026-07-14", "h2d-2026-07-14"),
        ("2026-07-16", "h2d-2026-07-16"),
        ("2026-07-23", "h2d-2026-07-23"),
    )
