...
This amendment closes exactly ten implementation ambiguities in the Owner-merged V-1A Amendment 3:
...
5. a resolver child that reaches or crosses a deadline enters irreversible failure quarantine and is killed, reaped and closed even though cleanup necessarily occurs after acceptance time has expired, while every separately authorized local-Git proof is forced into one exact non-network, non-replacement, non-lazy-fetch execution contract; and
...
9. the competing final-GitHub and final-database “immediately before receipt” requirements are replaced by one exact indivisible terminal-verification order that includes successful transaction/cursor/connection teardown before runtime remeasurement or evaluated-receipt construction; and
10. an Owner-side current-head prelaunch guard suppresses any recovery job when the retained seal already has an official terminal receipt, while a runtime terminal-exclusion guard prevents any pre-history failure or conflict from creating a forbidden second terminal receipt.

This amendment changes only the corresponding command, deadline, resolver-cleanup, final-checkpoint/final-database-authority/teardown ordering, already-terminal recovery reconciliation, dependency-ownership, time-zone-source and GitHub-transport-trust provisions of `ATOM-V1A-AMENDMENT-3-V1B-OPERATIONAL-PREREQUISITES-1`. V-1A, TLS Amendment 1, Amendment 2A and every other Amendment 3 rule remain controlling. In particular, this amendment changes no model, formula, numerical operation or order, benchmark, population, threshold, classification, multiplicity, session convention, causal cutoff, earliest-boundary rule, minimum, lineage/cohort rule, calibration, bootstrap/RNG stream, seal, one-look rule, recovery identity, receipt schema, status, stage, reason code, filename, source table, credential, role, service, command option, manifest membership, protected statistic or publication rule.
...
There is no separator, normalization, regenerated whitespace or newline between `HOOK_PREFIX` and `S`. Apply `T` exactly once to each of these four execution sites and nowhere else:
...
`GITHUB_RAW_WIRE_RESPONSE_MAX_BYTES` applies separately to each authenticated GitHub HTTP response. “Raw wire” here means every HTTP/1.1 response octet returned by the authenticated TLS stream after TLS decryption and before HTTP framing removal: status line, headers, the header terminator, transfer-coding syntax and trailers when present, and the encoded body. TLS record bytes that the application cannot observe are not double-counted. The exact content-encoding, framing and completion rules are defined below rather than inherited from an unspecified transport default.
...
Each request is exactly HTTP/1.1 and includes exactly one each of the literal header lines `Host: api.github.com`, `User-Agent: ATOM-V1B-READ-ONLY-VOLATILITY-SCORECARD-1`, `Connection: close` and `Accept-Encoding: identity`; no alternate casing, value, duplicate or environment/operator-derived User-Agent is accepted. It uses one fresh TLS connection for one request only. The TLS wrap sets `suppress_ragged_eofs=False`. The receiver reads the complete capped response through an authenticated TLS `close_notify` EOF before parsing it; a raw TCP/ragged EOF, TLS error, stall, deadline failure, second response, continuation after EOF or retained prefix is failure. Reject every informational `1xx`, redirect `3xx` and status outside the request's exact allowed-status set. No redirect, connection reuse, pipeline, upgrade or retry is accepted.

The response has exactly one HTTP/1.1 status line, one `CRLF CRLF` header terminator and no bare-LF, obsolete folding, invalid field name/value or `Location` header. For framing, accept exactly one of these mutually exclusive cases:

1. one `Content-Length` and no `Transfer-Encoding`, where the length is canonical ASCII decimal (`0` or a nonzero value with no leading zero) and equals exactly the number of wire-body octets before the already required clean TLS EOF;
2. one `Transfer-Encoding` and no `Content-Length`, where its comma-token sequence is exactly the single case-insensitive token `chunked`; every chunk-size line is one or more ASCII hexadecimal digits with no extension; every nonzero chunk has exactly that many octets plus `CRLF`; the terminal zero chunk is followed by exactly one empty trailer section (`CRLF`); and no missing, additional or trailing octet is accepted; or
3. neither framing header, in which case the body is close-delimited and completes only at the same clean authenticated TLS EOF.

Multiple `Content-Length`, `Transfer-Encoding` or `Content-Encoding` fields, both length and transfer framing, a transfer coding other than the single `chunked` token, any chunk extension or trailer, or inconsistent/truncated framing fails. `Content-Encoding` must be absent or exactly one case-insensitive `identity` token. No gzip, deflate, Brotli, compression, decompression or decoded expansion is authorized. Dechunking removes only the exact syntax above, so the resulting identity body is transitively bounded by the same raw-wire cap. The existing strict content-type, UTF-8 JSON, duplicate-key, schema, status and semantic validators then consume only that complete body under the applicable request deadline.
...
Exactly one resolver `Popen` is additionally authorized for each GitHub network request: Amendment 3's one resolver child with the exact absolute interpreter, isolated switches, fixed reviewed `-c` source, fixed file descriptors and exact secret-free environment. Its immediate pre-call observation must be timely. Its immediate post-call observation must be timely for success; if late, §5.1's quarantine is mandatory. No other resolver child, shell, executable, path, host, input, environment key or retry is authorized by this exception. The separately existing closed local-Git proof subprocess commands are not resolver children; each receives only §5.2's exact arguments/environment and positive remaining checkpoint time. Neither allowance creates a generic subprocess permission.
...
1. complete every required authenticated GitHub request, page, fixed-ID request and response-local validation under §4.1 except the one closing ruleset reread and one closing `refs/heads/main` reread reserved exclusively for step 3;
...
5. on the already open exact direct-host TLS connection and unchanged read-only snapshot, execute and fully validate exactly the second database-authority query required by V-1A §15.3, then synchronously complete in order the already entered transaction's successful `__exit__(None, None, None)`, cursor close and connection close, with no reconnect, new transaction, evidence read or other SQL; accept the step only when every operation and close has returned successfully, the transaction exit did not suppress, and the snapshot/context is marked fully closed so its later lexical exit performs no database protocol or network operation;
...
Steps 5–8 may begin only after steps 1–4 have succeeded completely. Step 5 retains the exact V-1A database-authority query, transaction and direct-host TLS law. V-1A supplies no separate database-query or teardown timeout literal, and this amendment invents none: immediately before and after the query, transaction exit, cursor close and connection close, observe the same monotonic checkpoint clock and require every observation to be strictly before the same checkpoint acceptance deadline. A return at or after expiry is discarded and fails the final checkpoint. The transaction exit and closes are lifecycle completion of the already authorized connection, not authority to execute another query, begin a transaction, reconnect, read evidence or derive an acceptance fact. They must finish before step 6 so the final runtime remeasurement observes the post-database process. The outer snapshot/context exit after this marked-successful close is a local no-op and must neither repeat a close nor touch the database.

If the query, transaction exit or a close does not return, no evaluated receipt, acceptance fact or further application work can occur; Amendment 3's existing protective job-cancellation, retained-seal and exact-recovery/consuming-negative rules govern, and a cancellation cannot create a timely result. If one fails after another has succeeded, irreversibly fail the checkpoint, perform only best-effort release of the already held remaining database resources under §3.2's quarantine-only cleanup rule, and never retry, reconnect or run SQL. A complete POST-EVALUATION AUTHORITY INVALID receipt may be constructed only after all still-held database resources are proved released; if release cannot be proved, application output cannot continue and the existing control-plane termination/incident obligation applies. No evaluated receipt is constructed before successful step-5 teardown, so a teardown defect can never replace or discard an already complete evaluated receipt.

Steps 6–7 receive a deadline-check callback tied to the same absolute checkpoint end; they may not use a fresh checkpoint, pagination or request deadline. A deadline crossed during a permitted local/native operation is detected by its immediate post-call check and fails the final checkpoint. No material GitHub, repository, source, ruleset, ref, database authority, provenance, GitHub-transport-trust or runtime validation may be deferred until after step 7. No protected value, receipt or PASS is accepted merely because an earlier part of the terminal sequence finished before the remainder expired.

This ordered sequence expressly replaces, only for final evaluated-receipt construction, both V-1A §15.3's statement that its second database-authority query runs “again immediately before evaluated receipt construction” and Amendment 3 §4's statement that no network operation intervenes between the final repository checkpoint and synchronous receipt construction. The complete step-5 lifecycle—exactly the required query followed by successful transaction exit, cursor close and connection close—is the sole permitted intervening database/network work. “Immediately before” for both inherited provisions now means membership in this one indivisible post-cell terminal-verification sequence: after truthful cells exist, no evidence read or protected calculation occurs; after step 5, no database resource remains open and no further database or other network operation occurs; after step 7, only the final monotonic check and existing in-memory receipt construction/output occur. The active no-ref-update window continues to prevent `main` movement throughout the sequence. This changes no query text, database fact, GitHub fact, calculation or receipt field.
...
### 4.3 Recovery prelaunch reconciliation and terminal-exclusion guard

An official terminal receipt can merge only after Amendment 3's original no-ref-update window is cleanly closed. That merge advances current `main` beyond the sealed execution SHA `E`, whereas exact recovery must keep its original build, local HEAD and execution identity at `E`. A recovery runtime therefore cannot lawfully change to the receipt descendant or satisfy a current-window `H == E` checkpoint while also discovering that later official receipt. This amendment authorizes no descendant execution, historical-build substitution or second recovery mode.

Before every recovery Create-job request, the Owner-controlled launcher must validate the retained seal exactly and, using the already authorized Owner GitHub control plane rather than a runtime credential or new source, acquire Amendment 3's exclusive ref/settings-writer gate and authenticate current default-branch head `H`, `E` as the retained seal's execution SHA and the complete first-parent interval from `E` exclusive through `H`. For every commit in that gap, authenticate the commit/parent/diff and its associated Owner-merged PR metadata under the incorporated history law. Completely enumerate, fetch, canonicalize, schema-validate, hash-validate and path-validate every official V-1B terminal receipt introduced anywhere in that interval as well as every terminal receipt present at `E` and `H`.

Independently paginate the repository's complete authenticated pull-request collection twice with the fixed query `state=all&sort=created&direction=asc&per_page=100&page=1`, following only its authenticated same-collection `rel=next` pages. Each pass must finish without truncation, duplicate ID/number, inconsistent envelope or pagination error. The complete PR-envelope collections are discovery inputs only: they are neither canonicalized nor compared as whole collections, and mutable fields unrelated to an official receipt cannot strand a retained seal. No time-window, state subset, author, label or path filter is supplied by the operator.

In each pass, for every PR whose authenticated `merged_at` is non-null and at or after Amendment 2A's adoption, fetch its complete paginated changed-file list. For every such PR containing any V-1B evaluated- or negative-receipt path, authenticate its repository/base ref, author and merger numeric identities and required logins, merge commit and parents, complete first-parent diff and receipt bytes even if that commit is no longer reachable from current `H`. A PR merged to a branch other than this exact repository's `main` is not official and cannot discharge a seal; validate that non-main classification but exclude it from the equality projection. A receipt-path PR merged to this exact repository's `main` must be the exact Owner-merged conforming one-file receipt PR or the guard fails.

From the successfully authenticated exact-repository/`main` receipt PRs in each pass, derive an array named `receipt_pr_projection`. Each member has exactly the keys `repository_id`, `repository_full_name`, `pr_id`, `pr_number`, `merged_at`, `base_repository_id`, `base_repository_full_name`, `base_ref`, `author_user_id`, `merger_user_id`, `merge_commit_sha`, `merge_parent_shas`, `merge_tree_sha` and `receipt_entries`. `receipt_entries` is the complete array of exact receipt-path additions sorted by UTF-8 path bytes, each with exactly `path`, `mode`, `git_blob_sha`, `raw_file_sha256` and `schema_version`. The separately required complete first-parent diff validation must succeed in each pass before a member is accepted; because a conforming official receipt PR adds exactly one `100644` receipt path and no other path, its immutable merge commit/tree/parent identities plus that exact receipt entry fully bind the validated diff without introducing a new, otherwise undefined diff-descriptor hash. All strings and integers retain their authenticated API/commit/file values; no title, body, label, comment/review count, `updated_at`, mutable head branch/SHA, unrelated PR or open/unmerged receipt-path PR enters this projection.

Sort `receipt_pr_projection` by numeric `pr_number` and require the canonical JSON bytes of the two projection arrays to be exactly equal. A member addition, removal or field mismatch fails without a third pass or retry. The complete raw PR collections may differ only in excluded metadata or excluded PRs; that difference is intentionally irrelevant, while any changed official-receipt operand remains fatal. Require each projected official receipt merge to occupy exactly one of two authenticated first-parent segments: either it is after Amendment 2A adoption and at or before `E`, with its exact file present byte-identically at both `E` and `H`; or it is in `E..H`, with its exact file present byte-identically at `H`. A valid earlier disjoint-manifest receipt is therefore preserved without being misclassified as a recovery-era merge. Any official receipt merge in neither segment is unreachable/invalid and blocks. Re-read default head after both complete discovery/projection passes and all PR/history/current-tree validation and require the same `H` before releasing the gate or deciding whether a job may be created.

A receipt path may appear in that interval only as the single added `100644` path of its conforming one-file receipt PR. No later commit may delete, modify, rename, replace, mode-change or cease to carry its exact authenticated bytes; require every introduced receipt to remain byte-identical at `H`. Reject an ancestry gap, force-rewrite, unclassified commit, hidden or unreachable official receipt merge, missing current path or any receipt-lifecycle mutation. The two complete PR discovery passes with their equal stable `receipt_pr_projection`, plus reachable-history and current-tree preservation proof—not a current-tree snapshot, mutable whole-PR collection or reachable commits alone—determine whether a terminal object ever became official. The check is one prelaunch safety guard, not a scorecard checkpoint, research read, receipt or identity field.

If that current history contains exactly one official terminal receipt in the supplied seal's same-manifest/intersecting-cell partition whose `manifest_id`, schema-derived complete ordered manifest cell set, non-null `readiness` and `seal_record_sha256` equal the retained seal exactly and whose complete inherited schema-aware seal/receipt cross-binding validates, the recovery obligation is already discharged: preserve and use that existing official receipt and do not create a recovery job. For an evaluated receipt, its `run_identity` must equal the retained seal's `run_identity`. For a consuming-negative receipt, its `sealed_run_identity` must equal that seal value; a POST-EVALUATION negative's top-level `run_identity` must also equal it. Every non-null causal top-level merge-identity projection must equal its sealed value; a null projection is accepted only where Amendment 2A §8.6's exact truthful-null exception requires it, never as a wildcard or mismatch. No replay command, new output, new file, changed timestamp/hash/path, new receipt or look is created.

If an official terminal receipt in that target partition is not the exact same-seal object above, or if that partition is duplicate, conflicting, noncanonical or otherwise invalid, do not create a job and require documentation-first incident disposition. Valid terminal receipts for wholly disjoint manifests/cells remain ordinary authenticated history and do not create target multiplicity. If the complete prelaunch check is unavailable or inconclusive, likewise create no job. No “latest,” favorable, matching-subset or caller-selected receipt may win.

Only a complete proof of zero same-manifest/intersecting-cell terminal receipts may proceed toward exact recovery, and then Amendment 3's original still-active no-ref-update window must independently remain current with `H == E`, the same manifest `M`, unchanged ruleset/history, retained build and every other recovery prerequisite. A descendant `H`, a closed/replaced window or a newly opened substitute window cannot be used merely because no receipt was found. Thus a conforming recovery remains possible only before the original consumed look has an official terminal receipt and while its original window/build obligation is still retained.

The runtime independently enforces the no-second-receipt property. From the first application point at which the supplied recovery seal has passed every existing local file/schema/canonical-byte/hash/manifest validation, arm a terminal-exclusion guard before any handler that could construct a receipt. Until the initial authenticated repository checkpoint has completely validated `H == E`, the unchanged active window and current official receipt history with zero intersecting terminals, any startup/runtime/provenance/GitHub/local-Git/approval/history failure opens no database/evidence resource, constructs no receipt, writes no file or stdout/stderr, exits 1 and leaves the retained seal incident-pending. It cannot enter generic consuming-negative routing or authorize a replacement look. If that checkpoint finds an intersecting terminal or invalid history, the same empty-output incident result applies.

Only after the complete zero-terminal checkpoint succeeds may the runtime disarm the terminal-exclusion guard and arm Amendment 2A §8.6's unchanged consuming recovery-failure routing for all later prerequisite, database, evidence, capacity, evaluation and final-verification failures. The active no-ref-update window makes that exclusion proof stable for the remainder of the invocation. This is the sole narrow override while terminal absence is unknowable; it never relabels the consumed identity as nonconsuming and never permits a second terminal receipt.
...
- if the acknowledgement byte has not yet been written, it must never be written after failure;
...
The parent tracks whether the one-byte acknowledgement was completely written. If failure occurs before that write, it closes the acknowledgement writer without a byte. If failure occurs during or after the completed write, it cannot retract or repeat the byte; it closes any still-open writer and treats the child as unaccepted even if the acknowledgement allowed it to begin exiting. In both states the parent discards resolver output/addresses without accepting a prefix, sends the hard-kill operation if the exact child is not already proven exited, synchronously reaps that exact returned PID, and closes the stdin/stdout pipes and pidfd. Deadline checks are not used to skip cleanup. Kill/reap/fd-close work that begins or completes after expiry is a quarantine-only exception to the acceptance deadline: it may do nothing except terminate and account for the already created child and release its fixed resources. It does not extend/reset a deadline, validate data, access a secret, emit output or permit success. The acknowledgement is only permission for that audited child to exit; it is never acceptance of its addresses. Timely exit, status, reap and descriptor closure remain required after the acknowledgement before any address or token use.
...
GIT_ALLOW_PROTOCOL     = empty string
```

`git` is resolved only through that exact system `os.defpath`, never the current directory, a caller path, `HOME`, `XDG_CONFIG_HOME` or another environment source. Its accepted executable remains inside Amendment 3's already trusted same-build Render system layer; this amendment adds no Git-binary hash or receipt field. Every invocation begins with this exact argument prefix before its already closed proof arguments:

```text
git --no-pager --no-replace-objects --no-optional-locks \
    -c credential.helper= -c protocol.allow=never -c core.commitGraph=false
```

The empty `GIT_ALLOW_PROTOCOL` is an empty transport whitelist; `protocol.allow=never` independently denies every protocol. `GIT_NO_LAZY_FETCH=1` forbids on-demand retrieval from a promisor remote. A missing object therefore fails locally and may not invoke a remote helper, fetch, retry or the authenticated HTTP client as a fallback. `GIT_NO_REPLACE_OBJECTS=1` plus the global option prevents replacement refs from changing commit, tree or blob interpretation; the fixed Linux null-device `GIT_GRAFT_FILE=/dev/null` prevents a legacy graft file from being loaded even if the repository path changes after preflight; and `core.commitGraph=false` forces history traversal to the authenticated commit objects rather than optional commit-graph parent data. Require `/dev/null` to be the existing canonical character null device before the first child. System/global configuration, prompting, optional locks and nonliteral pathspec interpretation are likewise disabled. The child receives `stdin=DEVNULL`, `stderr=DEVNULL`, captured bounded stdout and `close_fds=True` exactly as before; no pager, editor, hook, credential process, SSH command, proxy or transport-routing/helper variable beyond the deny-only `GIT_ALLOW_PROTOCOL` control is present.

The network-free provenance probe starts one 300-second local-source checkpoint immediately before its first predicate child below. That one immutable deadline covers all of the probe's predicate/Git-layout/graft checks, its exact `rev-parse HEAD` and validation of the returned source identity. It authorizes no resolver, PAT read, socket or other network operation and creates no new probe output field.

Each immediate pinned-Supabase-CA local Git blob proof or recheck required by incorporated TLS/database law that does not already receive an active GitHub checkpoint likewise starts exactly one immutable 300-second local-source checkpoint immediately before its first Git predicate. That same deadline covers the complete preflight, tree/blob/worktree-byte proof and pinned-CA equality for that one proof/recheck; no child or hash starts another clock. It grants the Git child no database URI, token or other secret and authorizes no resolver, socket, Git transport, evidence read or new database operation. It may run before the initial connection or while the one existing snapshot is open only where the incorporated immediate CA-recheck order already requires it; it cannot reset a database or research deadline or turn a late proof into acceptance.

Every initial, pre-seal, final and recovery repository checkpoint instead uses its already active unchanged 300-second GitHub checkpoint deadline. A pinned-CA proof invoked inside the shared final checkpoint also reuses that existing deadline rather than starting its local-source clock. Apart from the probe and those exact out-of-checkpoint pinned-CA proofs/rechecks, no local-Git call starts or extends another clock.

Before the probe's source-identity read and before the first local-Git proof in every initial, pre-seal, final or recovery repository checkpoint, run under the applicable deadline, prefix and environment the exact read-only predicates `rev-parse --show-toplevel`, `rev-parse --is-inside-work-tree`, `rev-parse --is-bare-repository`, `rev-parse --is-shallow-repository`, `rev-parse --show-object-format`, `rev-parse --path-format=absolute --absolute-git-dir` and `rev-parse --path-format=absolute --git-common-dir`. Require the first five outputs to be respectively the canonical current verified repository root, `true`, `false`, `false` and `sha1`; require the last two to be absolute, normalized, realpath-equal existing non-symlink directories returned as exact single-line paths. Freeze that exact git-dir/common-dir pair at the process's first proof and require equality at every later checkpoint. The common directory may be the ordinary repository `.git` directory or an already observed same-build linked-worktree common directory; it never becomes an operator input or a receipt field.

Before accepting any object/history output, inspect only `<common-dir>/info` and `<common-dir>/info/grafts` with immediate deadline checks and non-following `lstat`: `info`, when present, must be a realpath-equal non-symlink directory, and `grafts` must not exist in any file type, including as a symlink. An inspection error other than exact nonexistence fails. This explicit absence rule is required because `--no-replace-objects` does not disable legacy graft ancestry. No shallow-history acceptance, alternate object-format inference, graft, optional commit-graph interpretation or working-directory discovery is allowed. The existing exact `rev-parse HEAD`, commit/tree/blob/history and authenticated-GitHub equality proofs remain mandatory. The sole `git diff --name-only` proof additionally supplies, in this order before `--name-only`, `--no-ext-diff --no-textconv --no-renames --ignore-submodules=none`, so repository attributes or configuration cannot launch an external diff/text-conversion helper, suppress a changed gitlink or alter the closed path set through rename detection. No other local-Git command, option, environment, configuration query or repository mutation is authorized.
...
For one returned `PackagePath` object `P`, let `R = P.as_posix()` be its exact observable UTF-8 POSIX member name after `importlib.metadata.files()` has materialized it. `PackagePath` normalization may already have erased redundant raw CSV spellings such as `pkg/./file.py` or `pkg//file.py`; this amendment neither claims to recover or authenticate those pre-normalization `RECORD` field bytes nor rejects an otherwise valid member solely because that spelling was normalized before `P` was returned. No separate raw-`RECORD` parser or input is authorized. `R` itself must be nonempty and relative, contain no NUL, backslash, empty component or `.` component, and equal its POSIX-normalized spelling. A `..` component is not rejected merely because it exists: all `..` components must form one leading run, with no later `..`, and observable `R` must equal the exact canonical POSIX `relpath` from that distribution's canonical `locate_file("")` base to the final canonical target. This is the sole lexical-escape exception. It covers an installed wheel member moved by the installer into an exact `sysconfig` scheme destination, including a direct console-script target under `scripts`; it does not authorize a caller-selected parent traversal.
...
1. `HOOK_PREFIX` is exactly 60 ASCII bytes and is the first source bytes exactly once in probe, normal, recovery outer and decoded recovery child; both hooks are the same non-writing lambda and remain unchanged through entry; normal/recovery child length, hex length and SHA-256 equal §2.1; recovery rehexing is exact; all other command bytes/options remain unchanged; and representative bootstrap `OSError`, `KeyError`, failed `stat`/`sysconfig` lookup and explicit predicate refusal produce nonzero exit with empty stdout/stderr after `HOOK_PREFIX` executes.
2. The raw-wire and work-chunk literals equal 67,108,864 and 65,536; exact-cap plus timely authenticated TLS `close_notify` EOF passes; the next octet, raw/ragged/partial EOF or deadline equality discards the whole response; each fresh HTTP/1.1 request has exactly one each of the literal `Host: api.github.com`, `User-Agent: ATOM-V1B-READ-ONLY-VOLATILITY-SCORECARD-1`, `Connection: close` and `Accept-Encoding: identity` header lines and no alternate/duplicate User-Agent, reuse, redirect, retry or upgrade; and the three exclusive framing cases enforce canonical single `Content-Length`, extension/trailer-free single-token chunked syntax, or clean close-delimiting, with no mixed/duplicate framing or nonidentity content encoding. Local files are read/hashed in at-most-65,536-byte units with metadata/growth checks; immediate-pre/post-checked whole-response `bytes`, slice, join, copy, framing, dechunk, decode and parse operations accept only cap-bounded/transitively bounded operands; a late successful return is rejected; and no blocking network operation is introduced.
...
4. The final event order is complete GitHub/page/cross-endpoint/source validation, closing ruleset/ref validation, unaccepted repository-checkpoint candidate, the exact existing second database-authority query on the already open snapshot, successful transaction exit, cursor close and connection close, closing approved-CA/zone invariant validation, deadline-aware runtime remeasurement, final deadline check, then existing receipt construction. Test that this is one indivisible post-cell sequence; the complete query/teardown bundle is the sole database/network work after the closing ref, no evidence/protected calculation follows it, no database resource or network operation remains after it, and no material validation follows runtime remeasurement. The query, transaction exit and both closes have immediate monotonic pre/post checks under the unchanged checkpoint acceptance deadline; a simulated late return is rejected, a nonreturn cannot emit a receipt or continue application work, and the outer context exit is a non-network no-op after marked success. Do not assert or invent an inherited database timeout. A query/commit/close failure routes to POST-EVALUATION only after remaining resources are proved released and cannot replace a previously constructed evaluated receipt because construction has not begun. Runtime remeasurement cannot start early, reset the checkpoint or escape its deadline. Its fixed-root directory iteration, exact-distribution metadata, module/mapping traversal and sorting, platform/libpq/float/scalar reads, `dlsym`/offset work, local-file chunks and finite frozen-local-object operations all receive the §3.2 checks; an expiry during the database lifecycle or any permitted primitive routes to POST-EVALUATION AUTHORITY INVALID.
5. Resolver `Popen`, pidfd and fixed `/proc` operations have immediate monotonic pre/post checks. Test expiry before output, during either map read, before acknowledgement, during child exit/reap and on a late-returning `Popen`; pre-ack quarantine writes no acknowledgement, while post-ack quarantine never retracts or repeats the already written byte; both access no token, open no network socket, discard all addresses, create no second child, retry nothing and prove kill/reap/fd closure. Separately test each already authorized local-Git subprocess timeout/late return: discard stdout/status, kill and reap the exact child, close held resources, perform no later authority work and never retry. Freeze the exact local-Git environment and global argv prefix; prove the root/worktree/bare/shallow/object-format predicates, fixed common/git directories, null-device/graft absence and disabled configuration/commit-graph/transport sources; require the diff helper-disabling flags; simulate a promisor missing object, replacement ref and legacy graft and prove neither a fetch/transport/helper child nor substituted object/ancestry can be accepted. Prove the probe uses one 300-second local-source clock, each out-of-checkpoint pinned-CA Git proof/recheck uses exactly one of its own, and any proof inside an active repository/final checkpoint reuses that checkpoint without reset. Reap/close/release after expiry is cleanup only, requires no fictitious remaining deadline, and never changes failure to success or exposes a result. An unresolved child/resource prevents return to application work.
6. Every non-cache regular member returned by each exact distribution's `files()`/`RECORD` is canonically resolved and hashed. Test normal in-root members and leading-`..` members whose exact canonical `relpath` lands in each permitted install-scheme class, including generated direct `scripts` members. Reject an observable returned `R` that is absolute/empty/dot/cancellable/interior traversal, an escape outside all exact scheme roots, a noncanonical observable spelling, symlink component, non-regular/missing target, duplicate logical member or shared physical target. Prove the logical key retains exact observable `P.as_posix()` bytes but is never opened as a path; separately prove redundant raw `RECORD` spellings already normalized by `PackagePath` are not falsely claimed recoverable or rejected solely for that lost spelling, while duplicate normalized logical or physical claims still fail. Test loaded-origin/explicit-artifact outside-tree rejection, separate loaded-native mapping treatment and an inert unrelated scheme-root file that is neither attributed nor rejected solely by residence.
...
10. Validate prior evaluated and consuming-negative official receipts against their complete inherited schemas, authenticated Amendment-2A merge time, reader/verified-SHA domains, temporal ordering and exact New York close. Test the Owner-side prelaunch guard against a current authenticated descendant `H`: one schema-aware exact same-seal terminal discharges recovery without creating a job; a different-seal target receipt, target duplicate/conflict, invalid or unavailable history prevents job creation; lawful disjoint terminal receipts do not create target multiplicity; and evaluated/PRE/POST run-identity fields plus Amendment 2A's exact truthful-null merge projections are handled by their respective schemas. In the runtime, arm the terminal-exclusion guard immediately after local seal acceptance and prove that every failure before complete `H == E`/active-window/zero-target-terminal classification produces empty stdout/stderr, exit 1 and no receipt/file/database/evidence work; only a successful zero-overlap classification arms generic consuming-negative routing. No branch replays an existing receipt or substitutes descendant `H` for sealed `E`.

11. The Owner prelaunch reconciliation authenticates the complete first-parent interval `E..H`, every commit/parent/diff and associated PR, plus receipt trees at both endpoints. It also performs two complete passes of the fixed `state=all` PR discovery collection, derives and compares only the exact stable `receipt_pr_projection`, inspects every post-Amendment-2A merged PR with any receipt path, and brackets the result with identical default-head reads under the writer gate. Test a lawful pre-`E` disjoint receipt preserved byte-identically at both `E` and `H`, and a conforming `E..H` one-file receipt addition retained byte-identically at `H`; reject a later deletion, modification, rename, replacement, mode change, hidden/unclassified addition, ancestry gap or official receipt merge made unreachable by a force rewrite even when the current tree otherwise appears terminal-free. An added/removed/changed projection member fails without a third pass or retry, while a newly opened unrelated PR, an unrelated PR-head push, or a title/body/label/comment/review-counter update is excluded and cannot fail equality.
...
Preserve all V-1A, TLS Amendment 1, Amendment 2A and Amendment 3 law except the exact source transformation, deadline-work semantics, final-checkpoint/final-database-authority/teardown ordering, recovery prelaunch/terminal-exclusion reconciliation, installed-member ownership, forced time-zone source and GitHub transport-trust/probe-provenance extension above. The deadline durations remain 10/20/60/180/300 seconds. The resolver still has one child and the same address/output protocol. The no-ref-update window, credential and incident lifecycle, backend/restart proof, IPv4 cost gate, actual-seal capacity rule, one-off containment, nine commands, two scorecard CLI options, HOLD/WAIT/usage behavior, startup isolation, four-component runtime identity, direct-host database TLS, database-authority query/facts, one-snapshot scan, seal-before-results rule, first-complete-receipt duty and normal/rollback cleanup remain exact. Only the relative terminal placement and mandatory successful teardown of the second database-authority lifecycle are replaced by §4.2. Only §4.3's Owner-side no-job decision and the runtime guard before a complete zero-terminal proof take precedence over generic consuming-negative routing, solely to prevent a forbidden second terminal receipt; the ordinary exact recovery path and sealed `E` remain unchanged. Only the successful probe and `P` gain §6.3's one exact non-secret object; `A` binds that extended `P` without a new key, and no research identity, seal or receipt schema changes.
...
- Git execution environment, including `GIT_NO_LAZY_FETCH`, `GIT_NO_REPLACE_OBJECTS`, configuration-source controls and `GIT_ALLOW_PROTOCOL`: <https://git-scm.com/docs/git>
- Git protocol policy and replacement-object behavior: <https://git-scm.com/docs/git-config> and <https://git-scm.com/docs/git-replace>
