...
Each request is exactly HTTP/1.1, includes `Host: api.github.com`, `Connection: close` and `Accept-Encoding: identity`, and uses one fresh TLS connection for one request only. The TLS wrap sets `suppress_ragged_eofs=False`. The receiver reads the complete capped response through an authenticated TLS `close_notify` EOF before parsing it; a raw TCP/ragged EOF, TLS error, stall, deadline failure, second response, continuation after EOF or retained prefix is failure. Reject every informational `1xx`, redirect `3xx` and status outside the request's exact allowed-status set. No redirect, connection reuse, pipeline, upgrade or retry is accepted.
...
Independently paginate the repository's complete authenticated pull-request collection twice with the fixed query `state=all&sort=created&direction=asc&per_page=100&page=1`, following only its authenticated same-collection `rel=next` pages. Each pass must finish without truncation, duplicate ID/number, inconsistent envelope or pagination error; sort the completed observations by numeric PR number and require the two complete canonical collections to be byte-identical. A change or mismatch fails without a third pass or retry. No time-window, state subset, author, label or path filter is supplied by the operator.

For every PR whose authenticated `merged_at` is at or after Amendment 2A's adoption and whose complete paginated changed-file list contains a V-1B evaluated- or negative-receipt path, authenticate its repository/base ref, author/merger, merge commit, complete diff and receipt bytes even if that commit is no longer reachable from current `H`. A PR merged to a branch other than this exact repository's `main` is not official and cannot discharge a seal; retain it in the complete ledger but do not count it as a main receipt. A receipt-path PR merged to `main` must be the exact Owner-merged conforming one-file receipt PR or the guard fails. Require each such official receipt merge to occupy exactly one of two authenticated first-parent segments: either it is after Amendment 2A adoption and at or before `E`, with its exact file present byte-identically at both `E` and `H`; or it is in `E..H`, with its exact file present byte-identically at `H`. A valid earlier disjoint-manifest receipt is therefore preserved without being misclassified as a recovery-era merge. Any official receipt merge in neither segment is unreachable/invalid and blocks. Re-read default head after both PR-ledger passes and all PR/history/current-tree validation and require the same `H` before releasing the gate or deciding whether a job may be created.

A receipt path may appear in that interval only as the single added `100644` path of its conforming one-file receipt PR. No later commit may delete, modify, rename, replace, mode-change or cease to carry its exact authenticated bytes; require every introduced receipt to remain byte-identical at `H`. Reject an ancestry gap, force-rewrite, unclassified commit, hidden or unreachable official receipt merge, missing current path or any receipt-lifecycle mutation. The complete PR ledger plus reachable-history and current-tree preservation proof—not a current-tree snapshot or reachable commits alone—determines whether a terminal object ever became official. The check is one prelaunch safety guard, not a scorecard checkpoint, research read, receipt or identity field.
