...
**Decision ID (proposed):** `ATOM-SIM-4A-AMENDMENT-1-SIM5-DEADLINE-DRAIN-1`
**Status:** DRAFT — zero authority. Prepared by Claude under Amendment 1B §2 at Owner request (2026-09-07). ChatGPT Pro, or the Owner under the 1B fallback, is author of record. Nothing here is authorized before an Owner-approved merge.
**Amends:** `docs/sim-4a-exact-sim5-resolution-freeze.md` (`ATOM-SIM-4A-EXACT-SIM5-RESOLUTION-FREEZE-1`), file commit `4e6bd52`
...
**Phase:** SIM-5 (active pointer unchanged). Documentation-only. No implementation authority until merged.
...
The worker logs the existing `quote_admitted` counter with the same fixed-field, secret-free discipline as `SIM4_FAIL_CLOSED`: one line per telemetry interval when the value changed, of the form `SIM4_INGRESS status=<status> quote_admitted=<int> delta=<int>`. No new counter, no payload, no timestamp beyond the log's own, no configuration, URL, or exception text. SIM-3B §9's "every fail-closed event is observable through secret-free counters and logs" is extended to the success path so the admitted-quote rate per minute of regular hours becomes a number.
...
Implementation owner: per `AGENTS.md` active-phase pointer as merged — Codex preferred; Claude may assume and complete under Amendment 1D. One owner through completion; independent final-head review by someone other than the implementer; no self-review.
...
- Telemetry: the `SIM4_INGRESS` line is emitted only on change, carries only the fixed fields, and can never contain configuration, URLs, payloads, exceptions, credentials, or tokens.
...
Decision rule for the follow-on, stated now so it cannot drift:

- `RESOLVED` share at 30S/1M ≥ 90% and admitted rate p50 ≥ 5 quotes/s: closed; no window change.
- Admitted rate p50 below 1 quote/s: the resolver is starved upstream of the deadline. The next amendment addresses delivery or the SIM-3A queue/consumer, and any window change is sized from the measured rate and carries its own migration for `atom_v9_sim_resolutions_window_check`.
- Admitted rate high and `RESOLVED` share still low: §1's mechanism is falsified for the residual; return to the author of record with the numbers before any further change.
...
> SIM-5 decides each resolution from the complete set of quotes admitted inside its closed two-second window — drained through a deadline watermark exactly as SIM-4 entries are — labels an empty window as expired when one connection demonstrably covered it and as a gap otherwise, and logs how many quotes it admitted so the next decision is made on a number.

**END — proposed `ATOM-SIM-4A-AMENDMENT-1-SIM5-DEADLINE-DRAIN-1` (DRAFT, zero authority)**
