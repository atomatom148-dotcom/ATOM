...
**Author of record:** ChatGPT Pro, adopting this Claude-prepared draft with one narrow correction to the §7 decision rule (relayed by the Owner, 2026-09-08). Owner approval of this exact text is the Owner's merge of the pull request that carries it.  
...
This repair closes only when, on one complete regular session, both of the following hold: (a) the resolution-coverage threshold — `RESOLVED` share among regular-hours targets of at least 90% — is met at 30S and at 1M, each individually; and (b) the ingress-rate threshold — `quote_admitted` p50 of at least 5 quotes per second over regular-hours minutes — is met with complete telemetry, meaning one worker instance with no restart or deploy inside regular hours and `SIM4_INGRESS` present from open to close.
