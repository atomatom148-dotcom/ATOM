...
The worker logs the existing `quote_admitted` counter with the same fixed-field, secret-free discipline as `SIM4_FAIL_CLOSED`: one line at every telemetry interval (`SIM4_TELEMETRY_LOG_INTERVAL_SECONDS`, 1.000 s) while the worker status is `READY`, whether or not the value changed, of the form `SIM4_INGRESS status=READY quote_admitted=<int> delta=<int>`, where `delta` is the change since the previous interval and may be `0`. Outside `READY` admission is disabled and no line is emitted. Because it is unconditional while `READY`, the line is also the ingress heartbeat: a `READY` interval with no line is a telemetry gap, an interval with no line was not `READY`, and a minute of zero admissions is distinguishable from a minute the worker did not report. No new counter, no payload, no timestamp beyond the log's own, no configuration, URL, or exception text. SIM-3B §9's "every fail-closed event is observable through secret-free counters and logs" is extended to the success path so the admitted-quote rate per minute of regular hours becomes a number.
...
- Telemetry: the `SIM4_INGRESS` line is emitted at every interval while `READY`, including when `delta` is `0`, and never in any other status; it carries only the fixed fields and can never contain configuration, URLs, payloads, exceptions, credentials, or tokens.
