...
**Author of record:** ChatGPT Pro, adopting this Claude-prepared draft with one narrow correction to the §7 decision rule (relayed by the Owner, 2026-09-08). The Codex exact-head review findings on PR #328 (two rounds) — admission loss as an observation discontinuity, exact loss and registration instants, bounded overflow handling, late registration, the ingress heartbeat from admission enablement with minute-bucket completeness and a pre-open baseline, and the coverage denominator including overdue open entries — are incorporated in §2, §3, §6 and §7. Owner approval of this exact text is the Owner's merge of the pull request that carries it.  
...
- Telemetry: the `SIM4_INGRESS` line is emitted at every interval once admission is enabled — including while the status enum reads `RECOVERING` and including when `delta` is `0` — and never before admission is enabled; it carries only the fixed fields and can never contain configuration, URLs, payloads, exceptions, credentials, or tokens.
- Loss record: a mutex-contention refusal records the monotonic sample taken immediately after the failed acquire; a loss instant is applied to every pending window before it can be discarded; an overflow marks only pending windows overlapping the lossy interval and leaves a window entirely after it untouched; a registration whose `[target, registration]` overlaps the lossy interval has unproven coverage.
