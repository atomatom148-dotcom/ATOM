...
**Corrects PR:** #325 at exact reviewed input head `74f55586fee9d2dc72aec34620cc82ccf9068f7d`, which superseded `c25a4517fc46668630476f6748ba80263d334db5`, `caeae8c1be63a54c5926b56c8aa44eb04be247ff` and every earlier head. The commit that applies this complete replacement will supersede that input head and every earlier head; none can satisfy the replacement's review or merge gate.  
...
- `docs/v-1a-amendment-1-tls-trust-anchor.md` (`ATOM-V1A-AMENDMENT-1-TLS-TRUST-ANCHOR-1`), solely its hash-bootstrap cross-references, password-preservation and operational-setup restrictions; §5's no-new-credential sentence only for the exact §3 PAT; and §5's no-ingestion-interruption sentence only for the one exact §6.3 restart after the complete lossless-continuity predicate has passed;
...
- `FREEZE.md`, solely the **Clock-independence and continuous-evidence amendment** sentences that prohibit a database restart from interrupting the live writer or pausing ingestion, and only for the one exact §6.3 restart after that same predicate has passed; and
- `AGENTS.md`, solely its **Active phase pointer** prohibition on new credentials, to except the one exact §3 `ATOM_V1B_GITHUB_TOKEN` for this parallel V-1B workflow. `FREEZE.md`, `AGENTS.md` and their pointers remain byte-unchanged; no other rule or active-phase authority changes.
...
This document is complete for Amendment 3; it is not a list of instructions to edit an earlier draft. V-1A, TLS Amendment 1, Amendment 2A, `FREEZE.md` and `AGENTS.md` remain incorporated controlling law except for the express replacements in this document. `FREEZE.md` and `AGENTS.md` remain byte-unchanged. Upon Owner merge, §6.3 expressly supersedes `FREEZE.md`'s database-operation/no-interference and maintenance-does-not-stop-ingestion sentences, and TLS Amendment 1 §5's no-ingestion-interruption sentence, solely for one completed top-level **Restart project** action on `afyiydxbjgzaiswnbcyj`. The exception authorizes a bounded database-availability interruption, never evidence loss. It exists only while every conjunct of §6.3's lossless-continuity predicate is proved and remains true. It authorizes no second restart, writer/source/configuration change, accepted-observation deletion, truncation, sampling, cap, omission, backfill, imputation, timestamp repair, boundary movement or relaxation of any V-1A/TLS/2A mathematics. Every other sentence of those sources remains controlling. Section 3 separately supersedes only `AGENTS.md`'s Active phase pointer prohibition on new credentials and TLS Amendment 1 §5's no-new-credential sentence for the exact `ATOM_V1B_GITHUB_TOKEN`. Neither exception transfers or changes the active SIM-5 pointer. The already merged TLS Amendment 1 §5 parallel-V-1B implementation exception continues; §3 is its sole credential exception. No other credential, role, source, service, phase, SIM, HIST8, V-1C, V-2, broker or capital authority is added. The Owner expressly reserves every merge in this correction and the subsequent V-1B implementation/receipt sequence; no general delegated execution provision is used to bypass that reservation.
...
For the exact CPython 3.14.3 process define `q = <sys.base_prefix>/<sys.platlibdir>/python314.zip`, `u = <sys.base_prefix>/<sys.platlibdir>/python3.14`, `t = <sys.base_exec_prefix>/<sys.platlibdir>/python3.14`, and `y = t/lib-dynload`, using the literal `os.path.join` constructions in the command. The interpreter-created initial `sys.path` must equal exactly `(q, u, y)` in that order. Every one of `q`, `u`, `t` and `y` must be absolute and canonical; `u`, `t` and `y` must be real directories; and `q` must be absent even as a symlink. Before any helper import, the bootstrap deletes the nonexistent landmark from `sys.path`, removes any importer-cache entry for it, and requires the remaining path to equal exactly `(u, y)`. Only then may it import `sysconfig`; the returned real `stdlib` and `platstdlib` roots must equal `u` and `t` respectively. The reviewed repository root plus the interpreter's verified `purelib`/`platlib` roots are then appended in stable first-occurrence order. The bootstrap must not call `site.main()`, `site.addsitedir()`, process a `.pth` file, add a user-site path, use `PYTHONPATH`, or import `sitecustomize` or `usercustomize`. The verified repository root must equal the real working directory before job creation; a different, extra, reordered, relative, symlink-substituted or unverifiable root is rejected.

The literal first phase imports only frozen `os` and built-in `sys`; it requires `sys.__spec__.origin = "built-in"`, `os.__spec__.origin = "frozen"` and `os.path.__spec__.origin = "frozen"`, checks the exact path, version, flags and environment, and rejects every `_PYTHON*` key before importing or calling `sysconfig`. That order is frozen because `_PYTHON_SYSCONFIGDATA_NAME` and `_PYTHON_SYSCONFIGDATA_PATH` can otherwise select executable sysconfig data despite `-I`/`-E`. Only after removal of `q` and those checks may the bootstrap import the named standard-library helpers, derive paths, bind the launcher and insert a synthetic `quant` namespace package whose sole search location is the verified regular repository `quant` directory. It is constructed from the shown `ModuleSpec(..., loader=None, is_package=True)`; `module_from_spec` supplies CPython's standard namespace loader, so acceptance binds that resulting standard loader, `__package__ = "quant"`, the one exact `__path__`/`submodule_search_locations`, and absence of an origin/file rather than incorrectly requiring the resulting loader to remain `None`. This exact-head command deliberately does not execute tracked `quant/__init__.py`, whose eager import surface is not an authorized startup dependency; the initializer remains byte/source-bound and unedited. The namespace permits reviewed `quant.*` child imports without adding a file or widening the implementation allowlist. No other synthetic package, meta-path hook or loader is permitted.

The inline pre-import guard and the implementation's common startup-isolation guard both run before either runtime secret is read, copied or parsed. No repository or third-party application code is imported until the inline guard has passed. The scorecard module's import-time body may define/import reviewed code only: it must not read either runtime secret, open a connection, perform network I/O, dump the environment, log, inspect evidence or write a file; `provenance_probe_main()` and the existing scorecard `main()` invoke the common guard before any secret access. The guards require `sys.flags.isolated = 1`, `ignore_environment = 1`, `no_user_site = 1`, `no_site = 1`, `safe_path = 1` and `dont_write_bytecode = 1`; exact CPython 3.14.3 and `sys.pycache_prefix`; the exact `q/u/t/y` construction and initial/remediated path tuples above; archive absence; built-in/frozen bootstrap modules; absence of `site`, `sitecustomize`, `usercustomize` and any prior `quant` module before bootstrap; exact synthetic-parent state afterward; and the verified regular source and interpreter bindings. They reject any extra, reordered, relative, duplicate, missing or symlink-substituted bootstrap root. Equality of `purelib` and `platlib` is permitted only through the literal stable first-occurrence de-duplication; no appended bootstrap entry may preexist in or duplicate `(u, y)`.
...
CPython can import `encodings` while constructing the interpreter path and before executing `-c`; therefore a secret-bearing process's later `not os.path.lexists(q)` check is defense in depth, not the pre-start trust proof. The secret-free probe must first establish, without either runtime secret accessible, that the exact `q` is absent, that only `u` and `y` remain as live standard-library import roots after removal, and that `u`/`t`/`y` are covered by the approved `stdlib_tree_sha256` and `loaded_native_tree_sha256` according to the unchanged V-1A coverage rules. The retained `ATOM-V1B-STARTUP-ISOLATION-1` portion of the existing operational packet binds those exact path facts, the successful probe `J`, `B`, `/etc/ld.so.preload` absence, interpreter/native measurements, complete effective configured-environment absence, and current authoritative Render documentation or support evidence that a one-off for this service snapshots the selected successful build and its unchanged native runtime/system layer plus the then-configured environment. Independent review must accept same-`B` artifact/system-layer continuity—including continued `q` absence and exact approved directory bytes—before either secret is installed. Immediately before each secret-bearing Create job, the Owner revalidates through the control plane that `B` remains selected, no later build/deploy or configuration writer intervened, the only delta from the accepted secret-free environment is the two authorized secrets, every startup-injection key remains absent, and the exact isolated command is supplied. This is the complete feasible prelaunch proof; it explicitly does not claim a new per-job `/etc` or `/proc` read.

The trusted boundary is therefore the authenticated Render control plane's binding of the new job to the independently probed `B` and unchanged native runtime/system layer. If current vendor evidence cannot bind that layer and continued archive absence, if the platform can silently substitute either, or if `B`/configuration continuity is ambiguous, do not install/reinstall a runtime secret on a secret-free path and do not create a job on a same-build retained-secret path; V-1B is `BLOCKED`. Handle already provisioned secrets only under §5.4/§8 and existing protective authority; a valid pending recovery remains on its existing consuming incident path and is not “re-probed.” An actual secret-bearing process still checks `q`, `/etc/ld.so.preload`, interpreter and every process-visible injection fact before application secret access. Because the interpreter/native loader has already started, any such late mismatch is treated as possible credential exposure as well as startup failure: close without database/evidence access, preserve the applicable new-seal or consuming-recovery route, revoke the PAT under protective authority, keep the service suspended and require documentation-first disposition for the database credential. It is not retroactive proof that an unapproved startup path saw nothing. Within the accepted provider boundary, this closes the unbound standard-library archive landmark, Python/sysconfig path injection, user-site/customization hooks, shell initialization/function/trace controls, command-wrapper substitution, Git/proxy/trust-store override paths, dynamic-loader audit/preload/library-selection hooks and the system preload file before a secret-bearing process begins. It does not protect against a compromised Render control plane/kernel or approved CPython/native artifact.
...
Upon this amendment's Owner-approved merge, and only through its later implementation and operational gates, that exact variable containing the exact PAT below is the sole parallel V-1B exception to `AGENTS.md`'s Active phase pointer prohibition on new credentials and TLS Amendment 1 §5's no-new-credential sentence. Every other Active phase pointer and TLS Amendment 1 prohibition remains controlling. This exception neither takes the SIM-5 pointer nor authorizes another credential, role, service, source or phase.
...
After the exact Amendment 3 and implementation merges and the credential-free accepted build/probe, the Owner opens one window scoped to exactly one manifest `M`. For the first window on an approved build/source revision, and for every window after a later build/probe or credential replacement, that opening and its independent acceptance occur before either `ATOM_V1B_GITHUB_TOKEN` or `ATOM_E1_SCORECARD_READONLY_DATABASE_URL` is installed or reinstalled in or otherwise made accessible to the Render service. For a later sequential manifest window on the same still-approved `B` and `E`, §5.4 may retain the same already accepted PAT and rotated URI between windows: the prior window must first be lawfully closed, every earlier job must be terminal, no valid recovery may remain pending, and the base service must remain suspended with the exact inert command; no process may run or read either secret between windows. The Owner then opens and independently accepts the new window before submitting or creating the next one-off job. This same-build retained-secret path is the only exception to opening before secret installation; it authorizes no secret change, build/configuration drift, base resume, probe, scorecard process or hidden removal/reinstallation cycle.

Each window may contain its single new-seal attempt and only the exact recovery attempts needed to discharge a seal emitted by that attempt, or it may be opened solely to discharge one already-pending exact seal for `M`; it may never contain another manifest, new look or replacement identity. A later sequential window—whether for the same or a different `E` or `M`—is permitted only after the prior repository-wide window was lawfully closed, its exact ruleset was deleted, its immutable approval became `HISTORICAL_CLOSED` under §2.6, and the new window receives fresh independent review and Owner approval; an incident-closed window additionally obeys the no-seal/retry restriction below. Concurrent or overlapping windows are forbidden across the entire repository, not merely within one execution SHA or manifest. The Owner first verifies no merge queue entry, auto-merge, bot, workflow, direct push, administrative push or other accepted ref-update request is pending for `refs/heads/main`; authenticates the repository identity/default branch and current 40-lowercase-hex head `E`; and records the complete pre-window matching-ruleset state plus every applicable prior closure. Any queued or uncertain writer or still-active earlier rule must be resolved without updating `main` before the window opens.
...
The retained non-secret `ATOM-V1B-NO-MAIN-UPDATE-WINDOW-1` operational record binds the returned ruleset ID, its creation and last-update timestamps, its complete canonical write-authorized readback and digest, repository ID, exact ref, `E`, opening time, zero pending-writer observations, pre-window ruleset-state digest, invocation/recovery manifest `M`, and the actual Owner identity. It also binds the complete opening ruleset-history proof defined below. Read back the same active ruleset and authenticate `refs/heads/main == E` before installing/reinstalling either runtime secret on the first/later-build path, or before the next Create-job submission on the same-build retained-secret path. Independent operational acceptance must verify that record before the applicable installation/reinstallation or Create job; no job may exist during acceptance. Its exact non-secret projection `W` and digest are explicit fields of the §2.6 approval payload; the complete private/control-plane evidence remains bound by `control_plane_evidence_sha256`. Neither adds a receipt, seal or identity field.
...
```text
len(Q.encode("ascii"))       = 3032
len(Q.encode("ascii").hex()) = 6064
sha256(Q.encode("ascii"))    = 5c24ae7113db69c57436dafa42cb3b8aa9961ad14577e0729825fd50edf87d0c
```
...
The outer recovery process executes the same complete two-phase startup prefix as the normal command through construction of the exact synthetic `quant` namespace, including the exact initial `(q, u, y)` check, archive-absence check, removal of `q`, and `stdlib`/`platstdlib` binding. It imports no repository or third-party module and reads neither runtime secret before that prefix succeeds. It then stages the retained seal and replaces itself with a fresh isolated interpreter whose `-c` argument is the decoded exact normal-command body. That child therefore repeats the complete version, built-in/frozen bootstrap, flag, environment, archive/system-preload, path, `sysconfig`, interpreter, source and synthetic-parent checks before `runpy` loads any application module or either runtime secret can be read. Direct execution of `quant/volatility_scorecard.py` is forbidden.

The outer source contains no U+0027 single quote; the three angle-bracket substitutions remain inside double-quoted Python strings and cannot become shell redirections. The child receives `sys.argv = ["-c", "--manifest-id", manifest_id, "--recovery-seal-file", retained_seal_path]` before `runpy.run_module(..., run_name="__main__", alter_sys=True)`, preserving the normal module-execution semantics. Before creating the outer process, the control plane proves §2.4 startup-injection-key absence, exact command/configuration and continued selection of the accepted `B`; the retained provider-bound secret-free-probe baseline supplies the archive-absence, approved-root-byte, system-preload and launcher proof without an impossible new recovery probe or claimed control-plane `/etc` read. Both outer and child still recheck the live archive/system-preload/launcher facts before application secret access. The scorecard independently performs every original §10.2 file/schema/hash/manifest check before repository or database access. A prelude/startup failure or platform command-size limit authorizes no truncation, alternative payload source, automatic retry or new-seal fallback. Resolve the same retained-seal recovery under the existing failure/incident rules. No new durable store is created. Section 5.3 measures the fully substituted command and counts the embedded 6,064-byte bootstrap hex as fixed overhead; no earlier recovery-template measurement remains valid.
...
Password replacement prevents later authentication with the old password but does not terminate an already authenticated PostgreSQL session, and an ordinary role may change its own password from a surviving session. Section 6.3 therefore sets the immutable reader role NOLOGIN, performs two exact OID-selected termination passes for every visible reader client backend, proves visible reader/prepared/settings zeros, performs the one password replacement with an in-transaction pg_authid row-version proof, and then requires a provider-confirmed full restart of the exact Supabase project while NOLOGIN remains active and the URI remains absent. Stock PostgreSQL 17 does not publish every startup child in pg_stat_activity before authentication/LOGIN processing; no NULL usesysid scan or fixed timeout is treated as complete. Finalization instead requires the complete pre-action and post-action lossless-continuity proofs, the provider’s completed termination semantics, a strictly newer postmaster generation on the same control-system identity, exact persistence of the committed rotation row, and authenticated proof that the project has one primary and zero replicas/secondary instances before and after. Any mismatch in those continuity/restart/generation/persistence gates leaves the URI absent, incident OPEN and V-1B blocked; retain NOLOGIN only when an exact readback proves it, otherwise preserve/report the observed or unknown role state under §6.3. No setting reset, prepared-transaction action, catalog edit, new grant, guessed baseline or unlisted restart is authorized.
...
Evidence-capable activation requires the real finalized version, COMPLETED/non-null actual rotation timestamp, both exact visible-reader termination passes, all required reader/prepared/settings zeros, independently accepted pre-action and post-action lossless-continuity proofs, final independently reconciled empty route-(b) pending ranges, the completed full-project restart and new-generation/unchanged-rotation-row proof, authenticated one-primary/zero-replica proof, explicit pooler termination-or-containment proof, the updated conforming URI and independent acceptance of the incident disposition. §2.6 exposes only the opaque incident ID/finalization digest/completion timestamp and approval, never private incident contents. Missing finalization, any unavailable authoritative `D_submit_max`, `D_restart_max` or `D_post_max`, uncertain invalidation/restart, a nonempty/unreconciled pending range, changed rotation row, incomplete continuity/termination/project/pooler coverage or an unresolved wider-privilege/active-abuse finding blocks activation. Limited historical telemetry may be recorded and dispositioned; it cannot prove that no abuse occurred.
...
**Mandatory lossless-continuity predicate.** The restart request must not be submitted until every pre-action conjunct below passes. Unknown, stale, sampled, partial, inferred or merely “known” state fails. Before `t_submit`, the independent reviewer accepts every static operand, route proof, hard maximum and the exact arithmetic rule; the response-derived `t_submit` cannot be reviewed before it exists. The Owner then performs only the frozen observation, deterministic arithmetic and one confirmation below. The completed action-time record, including the observed `t_submit`, is retained for mandatory post-action independent audit; that later audit and the post-action proof cannot retroactively authorize a failed pre-action conjunct.

Let `t_guard_start` be the exact server-UTC lower bound captured before the final component-state rechecks below. Let `t_submit` be the exact server-UTC `restart_action_lower_bound` sampled when the existing query evaluates `clock_timestamp()` immediately before the single **Restart project** confirmation. Let `D_submit_max` be an independently proved hard maximum from that exact server evaluation instant through provider acceptance of the one request. It includes the remainder of query execution, result delivery, parsing, both fixed timestamp comparisons, the Owner's click, request transport and provider queueing/acceptance; a bound beginning at query completion or UI submission is insufficient. Let `D_restart_max` be the current authoritative Supabase maximum from request acceptance through every project database and required connection plane being writable; and `D_post_max` an independently proved hard maximum from that point through writer reconnection, replay/pending-range reconciliation and completion of the post-action continuity proof. An average, percentile/SLA, estimate, prior observed duration, UI timeout, health-poll timeout or undocumented assumption is not a maximum. Retain every source, observation time, scope, unit and canonical evidence digest.

For each inventoried input path `p`, establish exactly one safe horizon `h_p`. For route (a), `h_p` is the earliest exact UTC time at which that path can receive any eligible, delayed, corrected, replayed or externally triggered input, or at which one of its already accepted forecast/outcome/commit-proof obligations can become due. For route (b), `h_p` is the exact end of the independently proved interval during which its already deployed durable source outside the restarted project can accept the complete worst-case input/backlog without loss; every record accepted before `h_p` must remain durably retained after `h_p` if necessary, until its exact destination COMMIT is unambiguously acknowledged. Select one exact `t_safe_end` such that `t_safe_end <= min(h_p)` for every path. Before `t_submit`, compute and independently accept the exact UTC cutoff `t_submit_latest = t_safe_end - D_submit_max - D_restart_max - D_post_max`. The protected interval is half-open: `[t_guard_start, t_safe_end)`. An input or obligation at exactly a route-(a) horizon is outside that interval; no inference is made about time after `t_safe_end`.

1. An exhaustive authenticated inventory identifies every process, service, job, function, direct/pooler/PostgREST client, quote/Level-II path, evidence-outbox worker, forecast/outcome/commit-proof writer and retry/drain path that can accept an eligible observation or write to project `afyiydxbjgzaiswnbcyj`. For each, retain immutable service/process identity, frozen source/eligibility law, destination, queue/retry surfaces and exact pre/post continuity watermark. A list of known clients, aggregate connection count or repository-only search is not completeness proof.
2. Each path satisfies exactly one route throughout `[t_guard_start, t_safe_end)`: (a) its frozen source/eligibility law proves that no eligible input or due obligation can occur in that half-open interval; or (b) its already deployed source outside the restarted project durably captures every accepted input before acknowledgement, is non-dropping, non-overwriting and non-truncating for the complete worst-case interval and backlog, preserves stable identity/order/event time, and retains each record until the destination COMMIT and exact destination identity/hash are unambiguously acknowledged. A disconnect or ambiguous COMMIT leaves the source record durably pending; stable identity plus the existing destination constraint/reconciliation proof must prevent both loss and duplicate acceptance. `EVIDENCE_OUTBOX_CAPACITY = 256`, another bounded in-memory queue, best-effort retry, destructive lease, acknowledgement-before-durable-capture or unproved finite buffer does not qualify. Market closure alone is not route (a) unless the frozen source law excludes every late, corrected and unscheduled eligible input. No writer or source is paused, disabled or reconfigured to manufacture either route.
3. Before the final state observations, capture and retain `t_guard_start` with:

~~~text
SELECT to_char(pg_catalog.clock_timestamp() AT TIME ZONE 'UTC',
               'YYYY-MM-DD"T"HH24:MI:SS.US"Z"')
         AS continuity_guard_started_at
\gset atom_v1b_
SELECT to_char(
         :'atom_v1b_continuity_guard_started_at'::timestamptz
           AT TIME ZONE 'UTC',
         'YYYY-MM-DD"T"HH24:MI:SS.US"Z"'
       ) = :'atom_v1b_continuity_guard_started_at'
         AS continuity_guard_start_canonical;
~~~

Require the boolean true and retain the exact canonical 27-byte value. From that observation through completion of the restart proof, every route contract remains continuously effective.

4. After `t_guard_start`, re-read the complete inventory and each component's authoritative state. At its retained final observation, every writer must have zero accepted-but-not-durable observations, zero unprotected open or in-flight database writes, zero unprotected queued, leased, retrying or failed-but-retryable writes, and no already accepted obligation outside its route proof. Every route-(b) source must additionally have zero record accepted before `t_guard_start` still pending and zero unresolved pre-guard COMMIT ambiguity. Freeze the exact final post-guard source positions and destination watermarks as the reconciliation baseline; the required zeros mean no pre-guard record lies outside that baseline. The observations must be timestamped at or after `t_guard_start`. A route-(b) record accepted after a component's zero observation is conforming only while the durable source retains it through the exact COMMIT/reconciliation rule above; the earlier zero is not misrepresented as a continuously zero queue. Log-marker absence, sampled metrics, current `pg_stat_activity` or healthy status is insufficient.
5. Require a fully loaded, unfiltered and error-free authenticated Infrastructure view for exact project `afyiydxbjgzaiswnbcyj` to show exactly one Primary and zero Read Replicas or secondary PostgreSQL instances. Obtain the separate affirmative Owner confirmation for the exact disruptive action and complete independent review of the inventory, route assignments, bounds, post-`t_guard_start` observations and outage effects before capturing `t_submit`. Owner approval cannot waive a failed predicate. A loading, stale, filtered, partial or failed view is not a zero-replica observation; project metadata or service health alone is not replica inventory.
6. Capture `t_submit` using the existing `restart_action_lower_bound` query and require its existing generation boolean true. After that capture, no human selection, additional review, state discovery or configuration operation may intervene: only the Owner's mechanical application of the already independently reviewed exact byte arithmetic and the single confirmation are permitted. Require:

~~~text
t_guard_start < t_submit
t_submit < t_submit_latest
t_submit_latest = t_safe_end - D_submit_max - D_restart_max - D_post_max
~~~

The Owner performs only the two fixed timestamp comparisons against the already reviewed `t_submit_latest` and proceeds only while the independently bounded end-to-end submission mechanism remains within `D_submit_max`; otherwise do not click. The provider's retained action event must later prove `accepted_at <= t_submit + D_submit_max`, and the completed record receives the mandatory post-action independent audit. Equality at either strict comparison fails. Route (a) or (b) must remain proved for every path through provider acceptance. If query return, parsing or any later submission component stalls or lacks the preaccepted hard bound, or if any inventory, route, state, bound, timestamp, arithmetic or pre-action review is missing or changes, do not restart; keep `NOLOGIN`, keep the URI absent, leave the incident OPEN and keep V-1B `BLOCKED`.

Keep `NOLOGIN`, URI absence, parent suspension/inertness and zero jobs throughout. The completed action-time record includes the exact project and affected clients, current health, one-primary/zero-replica proof, `t_guard_start`, `t_submit`, pre-reviewed `t_submit_latest`, `t_safe_end`, all three hard maxima, route assignments, final component observations, exact arithmetic, pre-action independent-review evidence, Owner confirmation and post-action independent-audit disposition. This authorizes no remembered or blanket outage approval.

In the still-verified administrative session, after the final complete re-read and immediately before the Owner confirms the dashboard action, capture `t_submit` and recheck the original postmaster start time without replacing its baseline:
...
The retained `restart_action_lower_bound` is `t_submit`. Require it to be canonical, require `pre_restart_generation_unchanged = true`, mechanically evaluate both exact inequalities above, and then perform the one already approved confirmation through the bounded mechanism with no intervening action. The retained provider acceptance time must satisfy `accepted_at <= t_submit + D_submit_max`; a later or unprovable acceptance is a material continuity incident, not authority for a second request. Only through that sequence may the Owner use the authenticated Supabase General Settings page at:
...
Immediately before the final post-restart route-(a) event check and route-(b) source/destination reconciliation, capture the sole reconciliation cutoff from the same verified direct administrative session:

~~~text
SELECT to_char(pg_catalog.clock_timestamp() AT TIME ZONE 'UTC',
               'YYYY-MM-DD"T"HH24:MI:SS.US"Z"')
         AS reconciliation_cutoff
\gset atom_v1b_
SELECT to_char(
         :'atom_v1b_reconciliation_cutoff'::timestamptz
           AT TIME ZONE 'UTC',
         'YYYY-MM-DD"T"HH24:MI:SS.US"Z"'
       ) = :'atom_v1b_reconciliation_cutoff'
         AS reconciliation_cutoff_canonical;
~~~

Require the sole boolean true and retain the exact canonical 27-byte `t_reconciliation_cutoff`. It is an observed cutoff, not an operator-selected boundary.

**Mandatory post-restart continuity proof.** Prove: (1) provider acceptance occurred within `D_submit_max`; (2) the exact full-project action completed within `D_restart_max`; (3) every inventoried writer reconnected to the same project and the complete replay/pending-range reconciliation finished within `D_post_max`; (4) every pre-`t_guard_start` durable row, hash, commit proof and watermark prefix remains byte- and identity-equivalent; (5) every route-(a) path had no eligible input or due obligation throughout `[t_guard_start, t_reconciliation_cutoff)` and its current watermark equals its retained baseline; and (6) after revalidating the required zero pre-guard pending set and zero pre-guard COMMIT ambiguity, for every route-(b) path each record accepted in `[t_guard_start, t_reconciliation_cutoff)` is represented exactly once either by an identity/hash-verified destination COMMIT or by one still-retained durable pending record. A route-(b) destination watermark may advance only through the highest contiguous committed identity; the retained baseline prefix must remain unchanged, and the durable pending range must account exactly for every later uncommitted identity. “Pending” is accounted backlog, not an unexplained source/destination gap. Require zero unexplained gap, duplicate, truncation, expiry, overwrite, identity/event-time rewrite or ambiguous destructive acknowledgement. Every route contract remains continuously effective while these checks run; a route-(b) record accepted at or after `t_reconciliation_cutoff` is durably captured under that contract and joins the activation-blocking pending range unless already committed and included in the later final reconciliation.

Recheck every reader-backend, restart, pooler-containment and component-authority proof. Database health, a newer postmaster, reconnection or row persistence alone is insufficient. Immediately after every conjunct above has completed, with no intervening state discovery or discretionary action, capture the sole completion upper bound from that same session:

~~~text
SELECT to_char(pg_catalog.clock_timestamp() AT TIME ZONE 'UTC',
               'YYYY-MM-DD"T"HH24:MI:SS.US"Z"')
         AS recovery_complete_upper_bound
\gset atom_v1b_
SELECT to_char(
         :'atom_v1b_recovery_complete_upper_bound'::timestamptz
           AT TIME ZONE 'UTC',
         'YYYY-MM-DD"T"HH24:MI:SS.US"Z"'
       ) = :'atom_v1b_recovery_complete_upper_bound'
         AS recovery_complete_upper_bound_canonical;
~~~

Require the sole boolean true and retain that exact canonical 27-byte value as `t_recovery_complete`. Require `t_reconciliation_cutoff < t_recovery_complete < t_safe_end` and `t_recovery_complete <= accepted_at + D_restart_max + D_post_max` using only the already accepted exact UTC operands. A missing, late, noncanonical or reconstructed value fails; no earlier check time or operator clock may replace it.

If any bound or conjunct fails, is late or is unknowable after submission, record a material continuous-evidence incident; do not provision the URI, restore `LOGIN`, finalize the incident, repeat the restart, backfill, impute or repair evidence, and keep V-1B `BLOCKED` for a documentation-first decision. These are pre-invocation operational failures and do not change V-1A/Amendment 2A receipt or consuming-look routing.

A nonempty exact route-(b) pending range is losslessly accounted but is not V-1B evidence-ready. It remains an explicit continuity obligation and blocks operational approval and every evidence-capable V-1B job until the existing writer commits it under the frozen identity/order law and a final independent source/destination reconciliation—using a newly observed exact server-UTC cutoff and completion upper bound under the same two query forms—proves contiguous completion through the retained pending range. That ordinary commit is neither an authorized backfill nor a repair; expiry, overwrite, manual replay or a changed writer path remains forbidden.

Only after the restart/rotation-row/pooler and complete post-restart continuity proofs pass, install the same new credential in the exact Render reader URI using Save only while NOLOGIN remains true, keep the parent suspended/inert and create no job. Revalidate the exact role identity and attributes, exact persisted rotation row, newer generation, prepared/settings zeros, URI provisioning state and no intervening role/project writer. Then restore LOGIN in one guarded transaction:
...
The finalized private incident record retains: exact project/zero-replica observations before and after; the complete independently reviewed pre-action lossless-continuity predicate; exact `t_guard_start`, `t_submit`, `t_submit_latest`, `t_safe_end`, `t_reconciliation_cutoff` and `t_recovery_complete`; both canonical timestamp-query outputs; the evidence for `D_submit_max`, provider-bound `D_restart_max` and `D_post_max`; action-time outage confirmation and exact arithmetic; full-project control/confirmation and independently reviewable acceptance/completion evidence; the complete post-action prefix/watermark/pending-range reconciliation proof; pre/post postmaster times and stable system identifier; immutable role OID; NOLOGIN/LOGIN acknowledgements and canonical completion bounds; both termination-pass outputs and visible zero checks; every prepared/settings observation; pre-password retained rotation lower bound/xid, pre/post password-row booleans, commit status and post-COMMIT bound; all five post-reconnect input round trips; exact persisted-row proof; pooler disposition; URI provisioning; and final reader zeros. It retains no password, URI, verifier or verifier digest.
...
2. Restore the action-time non-secret configuration only while the base service remains suspended and only to the extent that doing so cannot strand a pending exact recovery. The base command remains exactly `python -I -S -B -X pycache_prefix=/dev/null/atom-v1b-no-pyc -c "raise SystemExit(0)"`. Do not restore the historical E-1 command even as a suspended configuration value during V-1B rollback; any later return to E-1 is outside this amendment. While any hash-valid seal lacks its first complete terminal receipt or any valid exact recovery remains pending, retain the exact sealed `B`, `PYTHON_VERSION=3.14.3`, `ATOM_V1B_AUTHORIZED_MAIN_SHA=E` and every other non-secret configuration value bound by that seal or required by the approved recovery command/environment; neither an earlier value nor absence may replace them. Only after independent review confirms that no such seal or recovery remains may the previous `PYTHON_VERSION`/authorized-SHA presence and value and any other recovery-bound non-secret setting be restored, without launching the reverted runtime. Never restore a §2.4 startup-injection key. Leave all untouched settings untouched.
3. Remove `ATOM_V1B_GITHUB_TOKEN` from this service and revoke the dedicated PAT through the Owner’s secure control plane only after independent review confirms that no hash-valid seal lacks its first complete terminal receipt and no valid exact recovery remains pending. While one remains pending, retain that same PAT only for the bounded recovery window allowed by §5.4; it may perform only the frozen authenticated recovery reads and must be removed/revoked immediately when that obligation closes. Do not install a replacement token or credential fallback. Existing immediate protective-revocation authority is preserved: if a security event requires revocation before recovery closes, revoke, keep the service suspended, preserve the build/seal/logs and classify the resulting inability to recover through the original consuming-negative/incident rules. Do not claim that exact recovery remains available, close the consumed look, or create a replacement look merely because the PAT was protectively revoked. This is the rollback path for the same conditional cleanup independently required on normal completion by §5.4.
...
1. Exact probe command/entry point and common startup guard: all six `sys.flags`; exact CPython 3.14.3; built-in `sys` and frozen `os`/`os.path`; initial `sys.path == (q,u,y)`; absent-even-as-symlink `python314.zip`; removal of `q` and its importer-cache entry before helper imports; exact remaining `(u,y)`; `sysconfig` `stdlib == u` and `platstdlib == t`; standard-library-before-repository-before-stable-deduplicated `purelib`/`platlib` ordering; `runpy`'s `-m`-equivalent `__name__`, `__spec__`, `__package__`, `sys.argv[0]` and option-tail behavior; and rejection of an existing archive, extra/reordered/relative/missing/symlink-substituted/preinserted roots, `PYTHONPATH`, user/global `sitecustomize`, `usercustomize`, executable `.pth`, `site` helpers and every forbidden process-visible startup key. Prove no module-import secret read, no post-start unsetting cure, absent-versus-empty runtime-secret rejection, no connection/network/evidence/dispatch output, complete common import/hash closure, exact JSON/file/stderr/exit rules and truncated-output rejection. The probe and evidence-capable mode cover identical artifact bytes without a CPU-specific probe dispatch baseline.
2. Authenticated same-build provenance; build-producing deploy identity rather than a fictional environment field; secret removal before Create job; secret-free probe observation of exact `q` absence, approved `u/t/y` byte coverage, `/etc/ld.so.preload`, `PATH` and resolved launcher; effective startup-injection-key absence; current provider evidence binding later one-offs to the accepted `B` native artifact/system layer including continued archive absence; and explicit rejection of a claimed control-plane future-job `/etc`/`/proc` read. Test that a live secret-bearing archive check is defense in depth because pre-`-c` imports are possible; linked-group/secret-file/blueprint/baked-source and empty-value rejection; Save-only installation of only the two secrets without rebuild or non-secret drift; exact pre-job control-plane revalidation; live inline mismatch as possible exposure; no secret-bearing parent resume; stale/changed/ambiguous build, archive or system-layer rejection; no re-probe while recovery is pending; and immutable build retention through recovery.
...
4. The inert parent, probe, all nine exact manifest commands, recovery outer process and recovery child carry exact `-I -S -B -X pycache_prefix=/dev/null/atom-v1b-no-pyc`; probe/normal/recovery carry the identical archive/root bootstrap; normal/recovery use the same direct synthetic-parent runner and only the original CLI options. Decode the fixed recovery-child hex and require exact equality, length and digest to the normal `-c` body. Render/parse actual complete synthetic seals including their final LF with the corrected longer recovery template; prove hexadecimal doubling and every wrapper/escaping/child-source/child-argv byte is counted. Test `command_bytes + 4096 == L` passes, one byte more fails, and missing/unverified limit fails before emission. Test long legal lineage/cohort fields without caps, no truncation/later-boundary substitution, unchanged successful seal bytes, the narrow new-seal capacity BLOCKED exception and consuming recovery failure. Do not assert a finite maximum seal size or reuse an earlier template measurement.
5. Retained PAT scope proof, missing/extra permissions, secret-redaction behavior and normal-completion removal/revocation, not just rollback. Test routine rollback while a hash-valid seal lacks its first terminal receipt: the same restricted PAT, sealed `B`, `PYTHON_VERSION=3.14.3`, `ATOM_V1B_AUTHORIZED_MAIN_SHA=E` and every other recovery-bound non-secret setting remain exact until recovery/terminal discharge; no earlier/absent value may strand recovery. Test that restoration/removal occurs only after independent no-pending-recovery proof, and that a security-required early PAT revocation preserves a consuming incident without claiming recovery or authorizing a replacement look. URI/password changes preserve the exact target/TLS tuple and never restore the exposed password.
6. Initial incident PENDING/null state permits containment/rotation but blocks activation; immutable reader OID; frozen NOINHERIT and temporary NOLOGIN; exactly two administrator-authorized reader-OID-selected visible-backend termination transactions; visible reader/prepared/settings zeros; one-primary/zero-replica Supabase inventory before and after; and the complete lossless-continuity predicate are mandatory. Test that `t_guard_start` is captured before every final component observation; future-`t_submit` facts are never used in an earlier check; `t_submit_latest` is frozen and independently accepted before `t_submit`; and equality or overrun in either `t_guard_start < t_submit` or `t_submit < t_submit_latest` prevents submission. Test exact equivalence of `t_submit_latest = t_safe_end - D_submit_max - D_restart_max - D_post_max`. Test that `D_submit_max` begins at the server's `clock_timestamp()` evaluation and includes query completion/result delivery/parsing/comparisons/click/request acceptance; a stalled or unbounded component fails before the click, and retained `accepted_at > t_submit + D_submit_max` creates the specified incident. Test exact canonical capture of `t_reconciliation_cutoff` before final reconciliation and `t_recovery_complete` immediately after every conjunct; missing/noncanonical/reconstructed output, reversed/equal ordering, `t_recovery_complete >= t_safe_end`, or capture outside `accepted_at + D_restart_max + D_post_max` fails. Test the half-open endpoint: route-(a) input before `t_safe_end` fails, while an input exactly at its proved horizon is outside the protected interval. Test incomplete writer inventory; accepted-but-undurable or unprotected open/in-flight/queued/leased/retrying/failed-retry work; a pre-guard route-(b) durable pending record or unresolved COMMIT ambiguity; an obligation outside its route proof; delayed/corrected/off-schedule input; attempted writer pause; the bounded 256-item in-memory outbox; and a missing, stale or non-end-to-end hard maximum. Test a route-(b) arrival and database-delivery attempt between the final zero observation and `t_submit`, disconnect before COMMIT acknowledgement, ambiguous COMMIT, durable pending retention, exact reconciliation, duplicate rejection, preserved baseline prefix and valid contiguous watermark advancement. Reject acknowledgement-before-capture, deletion on ambiguous COMMIT, unexplained pending gaps, destructive leases, expired/overwritten records, any post-`t_submit` human selection/additional review, and provider acceptance outside `D_submit_max`. Each prohibited or unproved state prevents restart; a late provider acceptance creates the specified material continuity incident and never authorizes a second request. Test pre-`t_submit` independent acceptance of every static route/bound/outage operand and `t_submit_latest`, Owner-only deterministic comparison of the observed `t_submit`, and mandatory post-action independent audit of the completed record; exact one-shot top-level Restart project submission with uncertain-outcome resolution; provider-confirmed termination of all ongoing workloads on the sole PostgreSQL instance; strictly newer pg_postmaster_start_time than the captured action lower bound with unchanged control-system identity; exact rotation-row persistence; route-(a) interval emptiness and unchanged watermark; and route-(b) identity/hash-verified COMMIT-or-exact-durable-pending reconciliation with preserved baseline prefix, contiguous committed advancement and zero unexplained gap/duplicate/truncation/expiry/overwrite/identity-time rewrite. Any late, missing or drifting post-action continuity proof prevents URI provisioning, LOGIN restoration and incident finalization. Test the password transaction’s pre-xmin mismatch and post-xmin match, full rotation xid8, committed status, sub-half-range unambiguous xid distance, SCRAM/NOLOGIN/OID predicates, cancelled/mismatched/no-op password paths, and the fact that psql emits no required ALTER ROLE tag. Test pre-password external capture of the canonical rotation lower bound/xid, exact capture/retention/post-restart rehydration and canonical round trips for the OID, xid8, pre-restart time, control-system identifier and restart-action lower bound, and exact canonical LOGIN-completion output retention. A lost/uncertain password COMMIT must stop before restart with no reconnect or repeat; a lost/uncertain LOGIN acknowledgement must remove the URI and preserve unknown role state with no extra reconnect/readback. Test a hidden pre-pgstat_bestart old-verifier process, a surviving role self-password change before restart, changed/frozen xmin, unavailable commit status, old/equal postmaster time, changed cluster, ambiguous/failed/duplicated restart, the adjacent Fast database reboot/Restart database control, request success/RESTARTING/ACTIVE_HEALTHY without the complete conjunction, primary availability before replica restart, a discovered replica/secondary, missing outage approval and loss of post-restart reconnect evidence. Preserve both exact visible termination passes, PID-reuse/collateral-termination incident handling, prepared transactions, role settings, explicit Shared/Dedicated/legacy pooler frontend/cache/server-lease disposition, fenced URI provisioning, guarded LOGIN restoration, post-LOGIN reader zeros, PUBLIC TEMPORARY acknowledgement and fail-closed OPEN/URI-absent handling that retains NOLOGIN only when proved and otherwise preserves/reports the observed LOGIN or unknown role state. Test the exact `pg_sleep(1.0::double precision)` statements as at-least-one-second separations, not exact elapsed-time claims; test every post-LOGIN final-proof failure without a silent NOLOGIN/LOGIN reassertion. Test that pg_stat_activity NULL usesysid scans and fixed authentication-timeout sleeps are rejected as completeness proof; no saved PID, third termination pass, second password rotation/restart, pooler mutation, grant, setting reset, replica deletion or automatic retry is permitted.
7. The no-ref-update-window contract requires an exact active no-bypass ruleset and `refs/heads/main == E` before initial or post-build secret installation/reinstallation, or before the next job when the same accepted secrets remain provisioned across a same-build sequential window; it remains required at every runtime checkpoint and through terminal discharge of any seal. Test exact `W` schema/hash and manifest binding; repository-wide complete `CURRENT`/`HISTORICAL_CLOSED` approval classification; a lawfully closed immutable older approval followed by one fresh current window; zero and multiple current windows including overlaps across different `E` or `M`; the fixed read-only ruleset GETs under the checkpoint deadline; omitted versus returned empty `bypass_actors`; both ruleset/ref observations; final ruleset/ref check immediately before receipt construction; pending writers and the exclusive writer gate through post-delete readback; missing/implicit bypass; unavailable/incomplete history or version evidence; persistent default-branch/ruleset/ref mismatch observable by the process; transient rule edit/reversion producing changed history, delete/recreate producing fixed-ID loss, and the invariant that no accepted `main` update is possible while the exact unchanged no-bypass rule is active; rejected versus effective bypass attempts; mixed observations; zero ref-movement rereads; one-manifest/single-look scope; window retention through recovery; exact closure evidence; prohibition on receipt/unrelated merges while active; runtime stage-correct failure routing; and the distinct post-terminal incident branches: no valid seal means no consumed look and no retry beyond existing law plus a fresh reviewed window, while a valid seal remains consumed/nonreplaceable and its first complete receipt keeps the unchanged Amendment 2A disposition. Test the same-build later-window path with prior lawful closure, terminal earlier jobs, no pending recovery, suspended inert parent, unchanged `B`/`E`/configuration and no process secret access; require fresh window review before Create without an invented removal/reinstallation cycle. Test that a later build/probe or credential replacement instead requires the original secret-free path and window acceptance before reinstall. A provably rejected request with no state change is audit-only and non-consuming; no descendant or “unrelated change” acceptance exists inside the window.
...
In particular, the exact 1H 121-window/21-full-session necessary lower floor remains necessary and not sufficient. No scientific, eligibility, acceptance, durability, population, scoring or cadence rule changes. Section 6.3's one expressly incorporated, lossless-gated availability interruption changes none of those rules and permits no missing eligible observation. No other change is authorized to V9/Q3 mathematics, existing source primitives, SIM, HIST8, evidence capture, V-1C, V-2, implied-volatility comparison, trading, broker access, risk limits or capital. Operational availability is not statistical `READY` and not evidence of profitability.
...
→ Owner opens and independent review accepts the initial or post-build no-ref-update window while both runtime secrets remain absent
→ temporary NOLOGIN + two exact reader-OID-selected visible-backend termination passes + reader/prepared/settings zero checks
→ one transaction-proved password rotation while fenced + independently accepted pre-action lossless predicate + exact one full-project restart + completed post-action writer/replay/watermark continuity + newer-generation/unchanged-rotation-row/zero-replica proof + pooler termination-or-containment + secure Save-only URI/PAT provisioning + exact LOGIN restoration + finalized incident
...
→ for a later same-build manifest only: retain the unchanged accepted secrets with the parent suspended/inert and no pending recovery, then open/accept its fresh one-manifest window before the next Create job; any later build/probe returns to the secret-free path
...
The sole Supabase database-restart operation is §6.3's Owner-confirmed dashboard action for exact project `afyiydxbjgzaiswnbcyj`. It is permitted only after the complete independently reviewed lossless-continuity predicate proves the exhaustive writer/client/input/retry inventory; a `t_guard_start` captured before all final component observations; post-guard zero unprotected state; half-open per-path no-input or external durable-replay coverage through `t_safe_end`; authoritative `D_submit_max`, end-to-end `D_restart_max` and `D_post_max`; and the exact strict `t_submit` arithmetic. A route-(b) record accepted after its final zero observation remains eligible only if captured durably before acknowledgement and retained through unambiguous destination reconciliation. `EVIDENCE_OUTBOX_CAPACITY = 256` and every other bounded in-memory/best-effort buffer are rejected. Immediately before and after the action, the Owner retains a fully loaded, unfiltered, error-free authenticated Infrastructure view proving exactly one Primary and zero Read Replicas/secondary PostgreSQL instances. Immediately before `t_submit`, the Owner retains the exact project, current health, every inventoried affected client, expected interruption, explicit action-time approval and independent acceptance of every static operand and the formula. The exact `t_submit` is then observed and substituted only into that frozen arithmetic, followed by the one confirmation; the completed record receives mandatory post-action independent audit. Approval cannot waive the predicate. The only submission is the top-level **Restart project** control plus its same-named confirmation exactly once at:

~~~text
https://supabase.com/dashboard/project/afyiydxbjgzaiswnbcyj/settings/general
~~~

**Fast database reboot**, **Restart database**, a service-only restart or an unavailable/disabled full-project control is rejected. An uncertain outcome is resolved by reading the same project state/event and, if needed, obtaining Supabase support confirmation tied to the exact project/action/time window; it is never resubmitted automatically. Request success, HTTP `200 {}`, toast, redirect, `RESTARTING`, transient disconnection or later `ACTIVE_HEALTHY` alone is not acceptance. The complete conjunction in §6.3 must prove provider acceptance within `D_submit_max`; exact action completion within `D_restart_max`; exact project health, before/after sole-instance inventory and a newer postmaster on the same system identifier; current documented full-project termination/restart semantics; and writer reconnection plus complete reconciliation within `D_post_max` and before `t_safe_end`. Route (a) requires interval emptiness and an unchanged baseline. Route (b) preserves the baseline prefix, advances only through a contiguous committed identity and accounts every remaining record as exact durable pending backlog, with zero unexplained gap or duplicate. No PID enumeration is claimed. No management endpoint, CLI, pause, restore, resize, failover, replica change or second restart is inferred. This project-wide action is credential-incident containment under the sole availability exception in §1.1, not evidence access, evidence loss or research execution.
...
| **A5 — Probe**: exact isolated §2.4 command in Create job only | Same `B`; documented local Render identity; exact flags/path/launcher; exact `q/u/t/y`, local runtime/artifact and `/etc/ld.so.preload` state; no arguments | No URI/PAT, startup hook, stdin, network, approval file, database or evidence | One exact probe JSON/LF plus retained non-secret archive/root-byte/launcher/system-preload observation, no file/stderr, exit 0; exact handled failure/usage outputs otherwise. Capture actual `J`, terminate and verify suspension. Never consumes a seal. |
| **A6 — Provenance/startup approval preparation**: build `P` and the §2.4 startup baseline from observation and authenticated `B` | Complete successful probe, independent build/source review, original canonical hashes and current provider same-artifact/system-layer evidence including continued `q` absence | Existing private acceptance packet; public mirror contains only defined non-secret fields/digests | Approved artifact/startup candidate binds archive absence plus `u/t/y` covered bytes before secrets; the later live check is defense in depth. No process can approve its own hashes and no control-plane `/etc` read is fabricated. No dispatch baseline. Wrong/absent evidence → BLOCKED, not a new runtime baseline. |
...
| **A8 — Freeze main and scope PAT evidence**: create/read back §4's exact temporary no-ref-update ruleset; on the initial/later-build path both runtime secrets remain absent, while a same-build later window retains the unchanged accepted secrets only with the parent suspended/inert, every prior job terminal and no pending recovery/process | Exact `E`, `M`, zero pending writers, pre-window ruleset state, applicable prior lawful closure, existing Owner GitHub control plane, independent reviewer, scoped token metadata | GitHub control plane/private packet only; no job exists. Initial/later-build URI/PAT remain absent; same-build later URI/PAT remain only in Render secret storage and no process can access them | Active no-bypass window record with `main == E`, interval-evidence capability and independent acceptance; non-secret PAT-scope record. Any ambiguity/bypass/pending writer blocks installation/reinstallation or the next Create job. |
| **A9 — Fence, terminate, rotate, losslessly restart, contain, provision and finalize**: exact §6.3 private psql NOLOGIN, two reader-OID-selected visible termination transactions, reader/prepared/settings checks, one xmin/xid8-proved password change, independently accepted pre-action continuity predicate, action-time-confirmed top-level Supabase Restart project, post-action writer/replay/watermark continuity, new-generation/unchanged-rotation-row/zero-replica proof, pooler disposition, Save-only URI/PAT, guarded LOGIN, final checks and incident finalization | Accepted §4 window; existing Owner admin/pg_signal_backend capability; PENDING incident; fresh password; exact CA/target/OID; authid SELECT; retained canonical OID/xid8/pre-restart time/control-system/restart-lower-bound tuple; authenticated one-primary/zero-replica inventory; exhaustive writer/input/retry state; exact `t_guard_start`/`t_submit`/`t_submit_latest`/`t_safe_end`; authoritative `D_submit_max`/`D_restart_max`/`D_post_max`; explicit Owner outage approval and independent review | Reader URI absent and NOLOGIN active through termination, rotation, the lossless-gated full restart, continuity, row/generation proof and pooler disposition; URI/PAT installed only after every proof while parent stays suspended/inert and no job exists | Before submission the complete predicate proves post-guard component state, half-open no-input or commit-retained durable-replay coverage and exact strict time arithmetic. After completion it proves route-(a) unchanged baselines and route-(b) preserved prefixes, contiguous committed advancement and exactly reconciled durable pending ranges. The remaining §6.3 conjunction proves every old primary backend ended and the persisted rotation row/pooler disposition. Any missing/mismatched/uncertain proof means no restart or no finalization as applicable, leaves the incident OPEN, removes/keeps the URI absent, blocks activation and authorizes no retry, second rotation or restart; retain NOLOGIN only when proved, otherwise preserve/report the observed LOGIN or unknown role state. |
...
| CPython inserts `<base_prefix>/<platlibdir>/python314.zip` and an archive appears there before a secret-bearing launch | No launch. The secret-free probe/accepted same-`B` system-layer record must prove exact `q` absence before either secret is installed; the secret-bearing process's later check is defense in depth because `encodings` may load before `-c`; §2.4. |
| The initial path has an extra/reordered root, `q` is a symlink, or `sysconfig`'s `stdlib`/`platstdlib` do not equal approved `u`/`t` | Bootstrap exits before application secret access. It accepts only `(q,u,y)`, removes `q` and its importer cache, then accepts only `(u,y)` before helper imports; §§2.4/5.2. |
| Isolated startup cannot import `quant`, so an operator calls `site.addsitedir()` or drops `-S` | Rejected. Only the literal archive-removing, exact-root `sys.path` bootstrap is authorized; it processes no `.pth` and all probe/parent/normal/recovery processes retain `-I -S -B`. |
...
| Manifest 1 closes cleanly on unchanged `B`/`E`, and its accepted PAT/URI remain provisioned for manifest 2 | This is the exact same-build retained-secret path: prove every prior job terminal, no pending recovery, suspended inert parent, no process secret access and the prior repository-wide closure; then open and independently accept manifest 2's fresh window before its Create job. Do not invent a secret removal/reinstallation cycle. Any build/probe or credential change instead returns to the secret-free path; §4. |
...
| Market is closed, queues look empty, project is healthy or prior restarts usually finish quickly | No restart. Section 6.3 requires the exhaustive authenticated inventory, `t_guard_start` before final component observations, half-open per-path no-input or commit-retained external durable-replay proof, all three hard maxima, a pre-reviewed `t_submit_latest` and both strict timestamp comparisons. None of these informal observations substitutes. |
| The only restart buffer is `EVIDENCE_OUTBOX_CAPACITY = 256`, another bounded in-memory queue or best-effort retry | No restart. It is not durable, uncapped, restart-surviving lossless replay for the complete worst-case interval/backlog; §6.3. |
| A route-(b) source has a record accepted before `t_guard_start` still pending, or an unresolved pre-guard COMMIT ambiguity | No restart. The final post-guard observation requires both sets empty before it freezes the source/destination reconciliation baseline; otherwise a pre-guard record could escape the protected interval's partition; §6.3. |
| A writer accepts route-(b) input after its final zero observation but before submission | Restart remains eligible only if the record was durably captured before acknowledgement and remains retained through unambiguous destination COMMIT/reconciliation; otherwise do not restart. The earlier zero is not relabeled as continuous queue emptiness; §6.3. |
| `t_submit` is captured before static independent review finishes, query/result handling lacks the hard bound from server evaluation, a reviewer is asked to make a new choice after `t_submit`, or `accepted_at > t_submit + D_submit_max` | No click in any pre-submission case. Late provider acceptance is a material continuity incident with no second request; the mandatory post-action audit cannot retroactively authorize it; §6.3. |
| Route-(b) replay advances the destination watermark or leaves a durable record pending | Preserve the pre-guard baseline prefix, advance only through the highest contiguous identity/hash-verified COMMIT, and account the remainder as the exact durable pending range. Accounted backlog is not an unexplained gap; deletion, expiry, overwrite, destructive acknowledgement or any unaccounted identity fails; §6.3. |
| The reconciliation cutoff or completion upper bound is missing, noncanonical, captured in the wrong order, reconstructed from an operator clock, or late | The post-restart proof fails. Only the two exact server-UTC observations may define `t_reconciliation_cutoff` and `t_recovery_complete`; require strict cutoff/completion/safety ordering and the accepted hard-bound arithmetic before URI provisioning, LOGIN or finalization; §6.3. |
| Restart completes but a route-(a) input occurs, a hard maximum is exceeded, or route-(b) has an unexplained gap/duplicate/truncation/expiry/overwrite/rewrite | Do not provision the URI, restore LOGIN or finalize the incident. Preserve the gap/unknown state as a material continuous-evidence incident and keep V-1B BLOCKED; §6.3. |
...
| Rollback begins while a hash-valid seal still lacks its first terminal receipt | Keep the same restricted PAT, sealed build, exact Python/SHA values and all other recovery-bound non-secret configuration solely for the bounded exact-recovery obligation; §8 steps 2–3 forbid routine restoration or cleanup from stranding recovery. If an actual security event requires immediate protective revocation, preserve and classify the consumed incident—do not claim recovery remains available or create a replacement look. |
...
The prior 125-assertion/27-transport-fixture authoring run applies only to the much earlier superseded text at `caeae8c1be63a54c5926b56c8aa44eb04be247ff`. Later checks attached to `74f55586fee9d2dc72aec34620cc82ccf9068f7d` likewise do not test this correction's archive-removal bootstrap, regenerated recovery child, lossless restart predicate or exact active-phase credential exception and are not evidence for the corrected final head. This correction claims no replacement assertion count or successful production execution.
...
| PAT not removed/revoked on normal completion | P2 | §5.4; §8 steps 2–3 | Normal closure and rollback both require Render removal plus GitHub revocation once no valid recovery remains; routine rollback retains the same restricted PAT and every recovery-bound non-secret value during a bounded pending recovery, while protective early revocation becomes a consuming incident rather than a false claim of recoverability. |
...
| CPython's default `python314.zip` landmark can precede the hashed stdlib roots and execute before the inline guard | Superseding exact-head P1 at `74f55586fee9d2dc72aec34620cc82ccf9068f7d` | §§2.4–2.5, 5.2, 9; A.2; B.1 | Secret-free same-`B` acceptance proves the exact archive absent and the only live stdlib/native roots byte-covered before either secret is installed. Every probe/normal/recovery bootstrap requires exact `(q,u,y)`, removes `q` before helper imports, binds `sysconfig` roots, and treats the later live check as defense in depth. |
| Mandatory full-project restart conflicts with incorporated continuous-evidence/no-ingestion-interruption law | Superseding exact-head P1 at `74f55586fee9d2dc72aec34620cc82ccf9068f7d` | §§1, 1.1, 6.1, 6.3, 9–10; A.1–A.2; B.1 | Exact one availability-only exception now expressly supersedes only the conflicting `FREEZE.md` and TLS Amendment 1 §5 sentences. A pre-guard timestamp, post-guard complete component checks, half-open no-input-or-commit-retained-replay coverage, three hard maxima and a pre-reviewed submit cutoff precede action; preserved baselines, contiguous committed advancement and exact durable-pending reconciliation precede incident finalization. Otherwise no restart. |
| Restart predicate used a future timestamp for prior facts, omitted pre-click query/result time and treated route-(b) pending work as both absent and gap-free | Prepublication P1/P2 caught in frozen-snapshot audit | §§6.1, 6.3, 9; A.1–A.2; B.1 | `t_guard_start` now precedes every final component observation; route coverage is half-open; `t_submit_latest` and all static operands are reviewed before the response-derived `t_submit`; `D_submit_max` begins at server evaluation and covers the complete path through provider acceptance; no later human selection enters the window; and route-(b) preserves the baseline prefix while separately reconciling contiguous committed advancement and the exact durable pending range. |
| Restart reconciliation/completion endpoints lacked an authoritative source and canonical capture order | Prepublication P2 caught in frozen-snapshot audit | §§6.1, 6.3, 9; A.2; B.1 | Two exact server-UTC queries now capture canonical `t_reconciliation_cutoff` before final reconciliation and canonical `t_recovery_complete` only after every post-action conjunct, with strict ordering and hard-bound comparisons; no operator clock or reconstructed timestamp may substitute. |
| Setup text could provision the rotated URI twice, and rollback could revoke/restore configuration while exact recovery remained pending | Prepublication P1/P2 caught in holistic lifecycle audit | §§2.5, 5.4, 8 steps 1–4, 9; A.2; B.1 | Initial setup installs each secret once while `NOLOGIN`; routine rollback retains the same PAT, sealed build and every recovery-bound non-secret value through terminal discharge. Security-required early revocation preserves a consuming incident and never creates a replacement look. |
| Every fresh manifest window demanded absent secrets while normal lifecycle retained the same PAT/URI across same-build manifests | Prepublication P1 caught in holistic lifecycle audit | §§2.5, 4, 5.4, 9–10; A.2; B.1 | First/post-build windows still precede installation or reinstallation. A same-build later window instead requires prior lawful closure, terminal jobs, no recovery, unchanged `B`/`E`/configuration and suspended inert/no-process containment, then fresh window acceptance before Create; it neither reads the secrets between windows nor invents a removal/reinstallation cycle. |
| Active SIM-5 pointer and TLS Amendment 1 §5 no-new-credential rules conflict with `ATOM_V1B_GITHUB_TOKEN` | Superseding exact-head P1 at `74f55586fee9d2dc72aec34620cc82ccf9068f7d` | §§1, 1.1, 3; C.2 | The exact PAT is the sole parallel V-1B credential exception after Owner merge; `AGENTS.md`, its pointer and TLS Amendment 1 remain byte-unchanged, and every other role/service/source/credential/phase prohibition remains controlling. |
...
| Unrelated `main` movement can make a sealed execution fail final `E == H` | Exact-head P1 | §§2.6, 4; §9 test 7 | Owner-enforced no-ref-update window holds exact `main == E` from before initial/post-build secret installation, or from before the next Create job on a same-build retained-secret sequential window, through terminal discharge. Runtime checkpoints detect persistent state loss; the mandatory post-terminal history audit detects transient edit/revert/delete/recreate. Authorized unrelated merges are deferred. A post-terminal breach with no valid seal consumed no look and any later window remains governed by existing eligibility/incident law; a breach after a valid seal leaves the first receipt's disposition unchanged, holds the consumed look and never permits a replacement identity. Lawfully closed sequential windows do not deadlock on their retained immutable approvals or require an unlisted secret cycle. |
...
| V-1A §§12–16 and TLS Amendment 1 | Existing implementation allowlist/conditional 033, exact CA bytes, direct hostname/port/database/role, full TLS tuple, catalog/no-write checks, original runtime field set, and TLS §5's no-new-credential and no-ingestion-interruption sentences | Preserved except §3's sole exact-PAT credential exception, the expressly named administration-prompt exception, and §6.3's one availability-only, lossless-gated restart exception to TLS §5. No scientific, source, TLS or evidence-durability rule changes. |
...
| `AGENTS.md` active pointer | Governance domain, SIM-5 pointer, no-new-credential rule, distinct documentation/implementation/operations and Owner merge | Pointer and file remain byte-unchanged. Amendment 3 expressly permits only the exact §3 read-only PAT for parallel V-1B after Owner merge; no other credential, role, service, source, phase or pointer authority changes. |
| `FREEZE.md` continuous-evidence law | Database operations do not interrupt the writer; maintenance does not stop ingestion; accepted observations remain durable | Section 6.3 expressly supersedes only those availability sentences for one restart and only while the lossless predicate remains true. It permits no lost/missed eligible input, evidence rewrite, cadence change or second restart; any missing hard maximum, route proof or continuity reconciliation means no restart/finalization. |
| Render one-off/CPython startup behavior | Build/configuration snapshot, command shell/loader/Python startup, default archive landmark, no inherited parent local disk, `startCommand` override, parent suspension not terminating a job | Secret-free launcher/system layer is bound before secrets, including exact `python314.zip` absence and approved stdlib/native-root bytes; configurable shell/native hooks are absent; probe/parent/normal/recovery use exact isolated commands; recovery staging and child are both covered. |
...
Operational acceptance is not asserted. In particular, no authoritative numeric Render startCommand capacity was established by the official references inspected for this document. `L` must be obtained through §5.3; until then evidence-capable activation is BLOCKED. No accepted hard values for `D_submit_max`, the end-to-end Supabase `D_restart_max` or `D_post_max` were established either; without all three, a pre-reviewed `t_submit_latest` and the complete pre/post lossless-continuity proof, the project restart is forbidden and activation remains BLOCKED. The same-build isolated probe, exact `python314.zip` absence and approved root-byte/system-layer continuity, effective startup-injection/system-preload absence, exact launcher, actual one-off identity/inheritance, secure transaction-proved password rotation, authenticated one-primary/zero-replica inventory, separately approved and provider-confirmed project restart, newer-generation/unchanged-rotation-row proof, real private incident finalization, pooler disposition, token scope/deadlined reads, direct-host TLS reachability and action-time IPv4 cost confirmation also require actual acceptance evidence. Those are fail-closed gates, not facts inferred from document text, synthetic tests or green CI. If vendor behavior cannot satisfy one frozen mechanism, redesign remains documentation-first.

---

## Source record and verification boundary

Authenticated read-only GitHub inspection on 2026-09-07 confirmed PR #325 open and unmerged, with base `f0035147a646fc7d4c7002c8a2706f4987f6a10c`, exact superseded input head `74f55586fee9d2dc72aec34620cc82ccf9068f7d`, and exactly one changed path: `docs/v-1a-amendment-3-v1b-operational-prerequisites.md`, Git blob `ab4e070406d9bfe26eca275c5cddf64dfab97427`. That head had incorporated the earlier corrections but is superseded because independent exact-head review found three additional P1s: the unbound CPython `python314.zip` startup landmark, the full-project restart's contradiction with incorporated continuous-evidence/no-ingestion-interruption law, and the exact PAT's contradiction with the active SIM-5 pointer's no-new-credential rule. This replacement resolves those three findings and their necessary command, recovery-hex, test, lifecycle and cross-contract references. Frozen-snapshot author and independent prepublication audits also corrected the restart timing/reconciliation model, authoritative endpoint capture, single secret-provisioning sequence and rollback/recovery dependency ordering; Appendix C records each disposition. Every other rule and finding disposition remains unchanged. The replacement's final head/blob/hash do not exist until Codex applies it, and no earlier review satisfies the new gate.
...
- `FREEZE.md`: Clock-independence and continuous-evidence law, including the no-writer-interference and maintenance-does-not-stop-ingestion sentences expressly and narrowly superseded by §6.3.
...
12. CPython 3.14 command line, path initialization, `site`, `runpy` and source path construction — https://docs.python.org/3.14/using/cmdline.html, https://docs.python.org/3.14/library/sys_path_init.html, https://docs.python.org/3.14/library/site.html, https://docs.python.org/3.14/library/runpy.html and https://github.com/python/cpython/blob/v3.14.3/Modules/getpath.py — isolated/no-site flags, pre-`-c` startup, the default `python314.zip` landmark even when absent, standard-library/dynload ordering, path/user-site/customization behavior and `-m`-equivalent module execution.
