...
| V9 15M, 30M, 1H | 250-pair withholding then MATURE | months |

**Hard arithmetic floor on V9 1H.** Non-overlap admits at most 6 calibration
pairs per session at 1H. The §7.2 latest-250 withholding therefore consumes at
least 42 sessions before `calibration_pairs` is non-empty, and MATURE requires
roughly 200 further effective observations. V9 1H cannot mature in under
approximately 15 weeks under any schedule. This is a structural consequence of
combining a fixed 250-pair holdout with a horizon whose non-overlap capacity is
six per session — the same class of defect as the 20-window warmup versus the
10-session floor, and it deserves the same explicit treatment.
