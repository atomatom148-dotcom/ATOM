-- 030: authorize E-1 scorecard reads for atom_historical_score_reader.
--
-- Controlling law: docs/e-1-evidence-scorecard-freeze.md (E-1B), section
-- "Authorized privilege exception", "Migration 030". This file contains
-- only that authorization. Apply as ONE transaction (Supabase MCP
...
-- created, no password is set or reset). No BYPASSRLS. No existing policy
-- is altered.
...
DECLARE
    ...
    named_policies integer;
    ro_executable integer;
    owners_ok integer;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_catalog.pg_roles
         WHERE rolname = 'atom_historical_score_reader'
           AND rolcanlogin AND NOT rolsuper AND NOT rolbypassrls
    ) THEN
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = 'E-1B precondition failed: atom_historical_score_reader must exist with LOGIN, NOSUPERUSER, NOBYPASSRLS';
    END IF;

    SELECT count(*) INTO write_grants
      FROM information_schema.role_table_grants
     WHERE grantee = 'atom_historical_score_reader'
       AND privilege_type IN ('INSERT', 'UPDATE', 'DELETE', 'TRUNCATE');
    IF write_grants <> 0 THEN
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = format('E-1B precondition failed: atom_historical_score_reader holds %s write grants', write_grants);
    END IF;
    ...
    IF select_tables <> ARRAY['public.atom_historical_replay_forecasts',
                              'public.atom_historical_replay_outcomes',
                              'public.atom_historical_replay_runs'] THEN
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = format('E-1B precondition failed: unexpected SELECT grants for atom_historical_score_reader: %s', select_tables);
    END IF;

    IF pg_catalog.has_schema_privilege('atom_historical_score_reader', 'atom_v9_internal', 'USAGE') THEN
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = 'E-1B precondition failed: atom_historical_score_reader already holds USAGE on atom_v9_internal';
    END IF;

    SELECT count(*) INTO named_policies
      FROM pg_catalog.pg_policies
     WHERE schemaname = 'public'
       AND tablename IN ('forecasts', 'forecast_outcomes', 'atom_v9_v4_forecasts', 'atom_v9_v4_outcomes')
       AND 'atom_historical_score_reader' = ANY(roles);
    IF named_policies <> 0 THEN
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = 'E-1B precondition failed: a policy on an evidence table already names atom_historical_score_reader';
    END IF;
    ...
    IF owners_ok <> 4 OR NOT EXISTS (
        ...
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = 'E-1B precondition failed: evidence tables and atom_v9_internal must be owned by postgres';
    END IF;
    ...
    IF membership_rows <> 1 OR matching_rows <> 1 THEN
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = format('E-1B precondition failed: postgres membership in atom_v9_proof_owner is not the single supabase_admin row (found %s rows, %s matching)', membership_rows, matching_rows);
    END IF;
    ...
    IF ro_executable <> 2
       ...
       OR NOT pg_catalog.has_function_privilege('supabase_read_only_user',
        ...
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = 'E-1B precondition failed: supabase_read_only_user must hold exactly the two 029 EXECUTE grants';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = 'E-1B precondition failed: atom_historical_score_reader can already execute a function in atom_v9_internal';
    END IF;
END
$e1b_assert_start$;
...
-- 7. Assert the exact final state.
DO $e1b_assert_end$
DECLARE
    membership_rows integer;
    matching_rows integer;
...
BEGIN
    ...
    IF membership_rows <> 1 OR matching_rows <> 1 THEN
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = format('E-1B final-state assertion failed: postgres membership in atom_v9_proof_owner changed (found %s rows, %s matching)', membership_rows, matching_rows);
    END IF;
    ...
    IF select_tables <> ARRAY['public.atom_historical_replay_forecasts',
        ...
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = format('E-1B final-state assertion failed: SELECT grants for atom_historical_score_reader are %s', select_tables);
    END IF;
    ...
    IF write_grants <> 0 THEN
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = 'E-1B final-state assertion failed: atom_historical_score_reader holds a write grant';
    END IF;

    IF NOT pg_catalog.has_schema_privilege('atom_historical_score_reader', 'atom_v9_internal', 'USAGE') THEN
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = 'E-1B final-state assertion failed: atom_historical_score_reader lacks USAGE on atom_v9_internal';
    END IF;

    SELECT count(*) INTO hsr_executable
      FROM pg_catalog.pg_proc p
      JOIN pg_catalog.pg_namespace n ON n.oid = p.pronamespace
     WHERE n.nspname = 'atom_v9_internal'
       AND pg_catalog.has_function_privilege('atom_historical_score_reader', p.oid, 'EXECUTE');
    IF hsr_executable <> 2
       OR NOT pg_catalog.has_function_privilege('atom_historical_score_reader',
              'atom_v9_internal.read_forecast_commit_proof(text)', 'EXECUTE')
       OR NOT pg_catalog.has_function_privilege('atom_historical_score_reader',
              'atom_v9_internal.read_legacy_evidence_publications_for_records(text, timestamptz, bigint[])', 'EXECUTE') THEN
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = format('E-1B final-state assertion failed: atom_historical_score_reader can execute %s functions in atom_v9_internal; exactly the two authorized are required', hsr_executable);
    END IF;
    ...
    IF ro_executable <> 0 THEN
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = format('E-1B final-state assertion failed: supabase_read_only_user can still execute %s functions in atom_v9_internal', ro_executable);
    END IF;
    ...
    SELECT count(*) INTO new_policies
     ...
     WHERE schemaname = 'public'
       ...
       AND roles = ARRAY['atom_historical_score_reader']::name[]
       AND qual = 'true';
    IF new_policies <> 4 THEN
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = format('E-1B final-state assertion failed: expected 4 permissive full-read policies, found %s', new_policies);
    END IF;
    ...
    IF preexisting <> 11 OR policy_total <> 15 THEN
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = format('E-1B final-state assertion failed: pre-existing policies %s of 11, total %s of 15', preexisting, policy_total);
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION USING ERRCODE = '42501',
            MESSAGE = 'E-1B final-state assertion failed: PUBLIC can execute a function in atom_v9_internal';
    END IF;
    ...
    LOOP
        IF EXISTS (SELECT 1 FROM pg_catalog.pg_roles WHERE rolname = role_name) AND (
            ...
            RAISE EXCEPTION USING ERRCODE = '42501',
                MESSAGE = format('E-1B final-state assertion failed: %s gained access', role_name);
        END IF;
    END LOOP;
END
$e1b_assert_end$;
