-- Select the newest timely proof rows inside each Phase E cohort window.
-- A late-proof incident must not let newer inadmissible rows shadow older
-- authoritative evidence. This migration mutates no evidence.
