#!/usr/bin/env python3
"""Dependency-free statement coverage gate for the Phase 1C-A adapter."""
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="atom-v2a-coverage-") as name:
        command = [
            ...
            name,
            "--ignore-dir",
            f"{sys.base_prefix}:/usr/lib",
            "--module",
            "pytest",
            "-q",
            "tests/test_v2a_external_parity.py",
            "-k",
            "not all_frozen_boundaries",
        ]
    ...
    executed = {
        line
        for line, text in enumerate(covered, 1)
        if text[:6].strip().rstrip(":").isdigit()
    }
    ratio = len(statements & executed) / len(statements)
    print(
        f"external_v2a_statement_coverage={ratio:.2%} ({len(statements & executed)}/{len(statements)})"
    )
    if ratio < 0.80:
        raise SystemExit("external V2A coverage is below 80%")
