"""E-1 read-only evidence scorecard (E-1A through E-1D).

Controlling law: ``docs/e-1-evidence-scorecard-freeze.md`` as amended by
E-1A through E-1D. Every statistic, count, selection rule, label, and guard
below is fixed by that document. The reader streams existing evidence through
one explicitly read-only ``REPEATABLE READ`` transaction, hydrates
admissibility only through the two authorized proof seams, computes the frozen
statistics as pure functions over in-memory rows, and prints one JSON receipt
to standard output. It never writes and may run at any hour under the
read-only fail-closed database contract.
"""
...
def verify_reader_identity(current_user: object) -> None:
    """Refuse any credential other than the dedicated E-1 role (E-1C/E-1D)."""
