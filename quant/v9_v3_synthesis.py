...
from quant.v9_v1_contract import (
    ...
    HORIZON_SECONDS, MAX_ACTIVE_AGE_SECONDS, V1Input,
)
...
def _eligible_v1_slot(slot: object, calibration: DirectionalCalibrationState,
    ...
    timestamps = (slot.forecast_cutoff_at, slot.source_as_of_at, slot.available_at)
    return (
        ...
        all(isinstance(item, datetime) and item.tzinfo is not None and
            item <= v1.cutoff_at for item in timestamps) and
        0.0 <= (v1.cutoff_at - slot.forecast_cutoff_at).total_seconds() <= MAX_ACTIVE_AGE_SECONDS
    )
